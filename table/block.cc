...
namespace leveldb {
  ...
  DecodeEntry(const char *p, const char *limit, uint32_t *shared,
  ...
  class Block::Iter : public Iterator {
   ...
   public:
    ...
    auto GetKeyString() -> std::string * override { return &key_; }

   private:
    void CorruptionError() {
      current_ = restarts_;
      restart_index_ = num_restarts_;
      status_ = Status::Corruption(Slice{"bad entry in block"});
      key_.clear();
      value_.clear();
    }
  ...
  Iterator *Block::NewIterator(const Comparator *comparator) {
    if(size_ < sizeof(uint32_t)) {
      return NewErrorIterator(Status::Corruption(Slice{"bad block contents"}));
    }

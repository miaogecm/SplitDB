...
namespace pmem {
  ...
  namespace obj {
     ...
     * satisfies all requirements of the Mutex and StandardLayoutType concepts.
    ...
    class mutex {
     public:
      ...
      void lock() {
        ...
        if(int ret = pmemobj_mutex_lock(pop, &this->plock))
          throw detail::exception_with_errormsg<lock_error>(
            ret, std::system_category(), "Failed to lock a mutex.");
      }
      ...
      void unlock() {
        ...
        if(ret)
          throw detail::exception_with_errormsg<lock_error>(
            ret, std::system_category(), "Failed to unlock a mutex.");
      }

...
namespace pmem {
  ...
  namespace obj {
    ...
    namespace experimental {
      ...
      /**
       * Persistent memory aware implementation of multi producer single
       * consumer queue.
       *
       ...
       *
       * @note try_consume_batch() MUST be called after creation of mpsc_queue
       * object if pmem_log_type object was already used by any instance of
       * mpsc_queue - e.g. in previous run of the application. If
       * try_consume_batch() is not called, produce may fail, even if the queue
       * is empty.
       *
       * Example:
      ...
      class mpsc_queue {
       public:
        ...
        template <typename Function> bool try_consume_batch(Function &&f);
      ...
      ptrdiff_t mpsc_queue::worker::acquire_cachelines(size_t len) {
      ...
      void mpsc_queue::worker::produce_cachelines() {
        ringbuf_produce(queue->ring_buffer.get(), w);
      }

      size_t mpsc_queue::consume_cachelines(size_t *offset) {
      ...
      void mpsc_queue::release_cachelines(size_t len) {
        assert(len % pmem::detail::CACHELINE_SIZE == 0);
        ringbuf_release(ring_buffer.get(), len / pmem::detail::CACHELINE_SIZE);
      }

      void mpsc_queue::restore_offsets() {
        /* Invariant */
        ...
        if(!pmem->written) {
          ...
          auto acq =
            w.acquire_cachelines(buf_size - pmem::detail::CACHELINE_SIZE);
          assert(acq == 0);
          (void)acq;
        ...
        acq =
          w.acquire_cachelines(pmem->written - pmem::detail::CACHELINE_SIZE);
        assert(acq == 0);
        (void)acq;
        w.produce_cachelines();
      }
       ...
       */
      inline pmem::obj::string_view mpsc_queue::pmem_log_type::data() {
      ...
      inline pmem::detail::id_manager &mpsc_queue::get_id_manager() {
        static pmem::detail::id_manager manager;
        return manager;
      }

      /**
       * Registers the producer worker. Number of workers have to be less or
       * equal to max_workers specified in the mpsc_queue constructor.
       *
       * @return producer worker object.
       *
       * @snippet mpsc_queue/mpsc_queue.cpp register_worker
       */
      inline mpsc_queue::worker mpsc_queue::register_worker() {
        return worker(this);
      }

      /**
       * Evaluates callback function f() for the data, which is ready to be
       * consumed. try_consume_batch() accesses data and evaluates callback
       * inside a transaction. If an exception is thrown within callback, it
       * gets propagated to the caller and causes a transaction abort. In such
       * case, next try_consume_batch() call would consume the same data.
       *
       ...
       * @note try_consume_batch() MUST be called after creation of mpsc_queue
       * object if pmem_log_type object was already used by any instance of
       * mpsc_queue. Otherwise produce might fail, even if the queue is empty.
       *
      ...
      inline bool mpsc_queue::try_consume_batch(Function &&f) {
       ...
       */
      inline mpsc_queue::worker &mpsc_queue::worker::operator=(worker &&other) {
        if(this != &other) {
          queue = other.queue;
          w = other.w;
          id = other.id;
       ...
       */
      template <typename Function>
      bool mpsc_queue::worker::try_produce(pmem::obj::string_view data,
                                           Function &&on_produce) {
        auto req_size =
      ...
      inline void mpsc_queue::worker::store_to_log(pmem::obj::string_view data,
                                                   char *log_data) {
 ...
 */
       ...
       */
      inline mpsc_queue::iterator mpsc_queue::batch_type::begin() const {
        return begin_;
      }
      ...
      inline mpsc_queue::iterator mpsc_queue::batch_type::end() const {
        return end_;
      }

      mpsc_queue::iterator::iterator(char *data, char *end)
          : data(data), end(end) {
        auto b = reinterpret_cast<first_block *>(data);
      ...
      void mpsc_queue::clear_cachelines(first_block *block, size_t size) {
        assert(size % pmem::detail::CACHELINE_SIZE == 0);
        assert(pmemobj_tx_stage() == TX_STAGE_WORK);

        auto end =
          block + static_cast<ptrdiff_t>(size / pmem::detail::CACHELINE_SIZE);
      ...
      mpsc_queue::iterator &mpsc_queue::iterator::operator++() {
        auto block = reinterpret_cast<first_block *>(data);
        assert(block->size != 0);

        auto element_size = pmem::detail::align_up(
          block->size + sizeof(block->size), pmem::detail::CACHELINE_SIZE);
      ...
      mpsc_queue::iterator::seek_next(mpsc_queue::first_block *b) {
        ...
        while(b < e) {
          if(b->size == 0) {
            ...
            auto size = b->size & (~size_t(first_block::DIRTY_FLAG));
            auto aligned_size = pmem::detail::align_up(
              size + sizeof(b->size), pmem::detail::CACHELINE_SIZE);

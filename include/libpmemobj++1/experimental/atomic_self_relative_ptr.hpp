  ...
  struct atomic<pmem::obj::experimental::self_relative_ptr<T>> {
   ...
   public:
    ...
    exchange(value_type desired,
             std::memory_order order = std::memory_order_seq_cst) noexcept {
      auto new_offset = accessor::pointer_to_offset(ptr, desired.get());
      auto old_offset = accessor::get_offset(ptr).exchange(new_offset, order);
      return value_type{accessor::offset_to_pointer<T>(old_offset, ptr)};
    }

    bool compare_exchange_weak(value_type &expected, value_type desired,
                               std::memory_order success,
                               std::memory_order failure) noexcept {
      auto expected_offset = accessor::pointer_to_offset(ptr, expected.get());
      auto desired_offset = accessor::pointer_to_offset(ptr, desired.get());

      bool result = accessor::get_offset(ptr).compare_exchange_weak(
        expected_offset, desired_offset, success, failure);
      if(!result) {
        try {
          expected = accessor::offset_to_pointer<T>(expected_offset, ptr);
        }
        catch(...) {
          std::terminate();
        }
    ...
    value_type operator++(int) noexcept { return this->fetch_add(1); }
    ...
    value_type operator--(int) noexcept { return this->fetch_sub(1); }
...
namespace pmem {
  ...
  namespace detail {
    ...
    /**
     * pmem::detail::can_do_snapshot atomic specialization for

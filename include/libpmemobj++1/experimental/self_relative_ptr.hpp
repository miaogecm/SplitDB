...
namespace pmem {
  namespace obj {
    namespace experimental {
      ...
      template <typename T> class self_relative_ptr;
      ...
      class self_relative_ptr<void> : public self_relative_ptr_base {
       ...
       private:
        difference_type self_offset(element_type *ptr) const noexcept {
       ...
       * self offsetted pointer and provides member access, dereference and
       * array access operators.
       *
       * @includedoc shared/pointer_requirements.txt
       *
       * Casting to self_relative_ptr_base can be easily done from any
       * self_relative_ptr<T> objects, but when casting between convertible
       * objects be advised to use constructors or operator= specified for such
       * conversion, see:
       * * self_relative_ptr::self_relative_ptr(self_relative_ptr<U> const &r) ,
       * * self_relative_ptr<T> & operator=(self_relative_ptr<Y> const &r) .
       * When casting indirectly with (void *) or using static_cast, and then
       * casting to the second (convertible) type, the Offset will be
       * re-calculated.
       *
      ...
      class self_relative_ptr : public self_relative_ptr_base {
       public:
        ...
        self_relative_ptr(PMEMoid oid) noexcept
            : self_relative_ptr_base(
                self_offset(static_cast<element_type *>(pmemobj_direct(oid)))) {
        }
         ...
         */
        operator persistent_ptr<T>() const { return to_persistent_ptr(); }
         ...
         */
        inline self_relative_ptr<T> &operator++() {
          detail::conditional_add_to_tx(this);
         ...
         */
        inline self_relative_ptr<T> operator++(int) {
          auto copy = *this;
          ++(*this);
         ...
         */
        inline self_relative_ptr<T> &operator+=(std::ptrdiff_t s) {
          detail::conditional_add_to_tx(this);
       ...
       */
      template <class T>
      inline void swap(self_relative_ptr<T> &a, self_relative_ptr<T> &b) {
        a.swap(b);
      }
      ...
      inline bool operator==(self_relative_ptr<T> const &lhs,
      ...
      inline ptrdiff_t operator-(self_relative_ptr<T> const &lhs,

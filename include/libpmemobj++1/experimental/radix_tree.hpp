...
namespace pmem {
  ...
  namespace obj {
    ...
    namespace experimental {
      ...
      template <typename T, typename Enable = void> struct bytes_view;

      /**
       * Persistent associative, ordered container with API similar and
       * partially compatible with the API of std::map.
       *
       * It is based on
       * https://github.com/pmem/vmemcache/blob/master/src/critnib.h The
       * implementation is a variation of a PATRICIA trie - the internal nodes
       * do not store the path explicitly, but only a position at which the keys
       * differ. Keys are stored entirely in leaves. More info about radix tree:
       * https://en.wikipedia.org/wiki/Radix_tree
       *
       * Unlike std::map radix tree does not use comparison (std::less or
       * equivalent) to locate elements. Instead, keys are mapped to a sequence
       * of bytes using user-provided BytesView type. The key's bytes uniquely
       * define the position of an element. In some way, it is similar to a hash
       * table (BytesView can be treated as a hash function) but with sorted
       * elements.
       *
       * The elements' ordering is defined based on the BytesView mapping. The
       * byte sequences are compared using a function equivalent to
       * std::string::compare.
       *
       * BytesView should accept a pointer to the key type in a constructor and
       * provide operator[] (should return a byte at the specified position in
       * the byte representation of value) and size (should return size of value
       * in bytes) method. The declaration should be as following:
       *
       * @code
       * struct BytesView {
       ...
       * @endcode
       *
       * By default, implementation for pmem::obj::basic_inline_string<CharT,
       * Traits> and unsigned integral types is provided. Note that integral
       * types are assumed to be in little-endian.
       *
       * Iterators and references are stable (are not invalidated by inserts or
       * erases of other elements nor by assigning to the value) for all value
       * types except basic_inline_string<CharT, Traits>.
       *
       * In case of basic_inline_string<CharT, Traits>, iterators and references
       * are not invalidated by other inserts or erases, but might be
       * invalidated by assigning new value to the element. Using
       * find(K).assign_val("new_value") may invalidate other iterators and
       * references to the element with key K.
       *
       * swap() invalidates all references and iterators.
       *
       * MtMode enables single-writer multiple-readers concurrency with read
       * uncommitted isolation. In this, mode user HAS TO call
       * runtime_initialize_mt after each application restart and
       * runtime_finalize_mt before destroying radix tree.
       *
       * Enabling MtMode has the following effects:
       * - erase and clear does not free nodes/leaves immediately, instead they
       * are added to a garbage list which can be freed by calling
       * garbage_collect(),
       * - insert_or_assign and iterator.assign_val do not perform an in-place
       * update, instead a new leaf is allocated and the old one is added to the
       * garbage list,
       * - memory-reclamation mechanisms are initialized.
       *
       * @note By default, concurrency is not enabled (it is not allowed to
       * perform concurrent operations on radix tree).
       *
      ...
      template <typename Key, typename Value,
                typename BytesView = bytes_view<Key>, bool MtMode = false>
      class radix_tree {
        template <bool IsConst> struct radix_tree_iterator;
       ...
       public:
        ...
        template <class InputIt> radix_tree(InputIt first, InputIt last);
        ...
        template <typename P,
                  typename = typename std::enable_if<
                    std::is_constructible<value_type, P &&>::value, P>::type>
        ...
        friend std::ostream &
        operator<<(std::ostream &os, const radix_tree<K, V, BV, Mt> &tree);
       ...
       private:
        ...
        template <typename K> leaf *internal_find(const K &k) const;
        ...
        std::pair<bool, const leaf *>
        next_leaf(Iterator child, pointer_type parent) const;

        template <bool Direction> const leaf *find_leaf(pointer_type n) const;

        static unsigned slice_index(char k, uint8_t shift);

        template <typename K1, typename K2>
        static byten_t
        prefix_diff(const K1 &lhs, const K2 &rhs, byten_t offset = 0);
        ...
        template <typename K> static BytesView bytes_view(const K &k);
        ...
        template <typename T> void free(persistent_ptr<T> ptr);
      ...
      struct radix_tree<Key, Value, BytesView, MtMode>::leaf {
        ...
        static persistent_ptr<leaf>
        make(pointer_type parent, const Key &k, const Value &v);

        template <typename K, typename... Args>
        static persistent_ptr<leaf>
        make_key_args(pointer_type parent, K &&k, Args &&...args);

        template <typename K, typename V>
        static persistent_ptr<leaf>
        make(pointer_type parent, detail::pair<K, V> &&p);

        template <typename K, typename V>
        static persistent_ptr<leaf>
        make(pointer_type parent, const detail::pair<K, V> &p);

        template <typename K, typename V>
        static persistent_ptr<leaf>
        make(pointer_type parent, std::pair<K, V> &&p);

        template <typename K, typename V>
        static persistent_ptr<leaf>
        make(pointer_type parent, const std::pair<K, V> &p);

        static persistent_ptr<leaf>
        make(pointer_type parent, const leaf &other);
      ...
      struct radix_tree<Key, Value, BytesView, MtMode>::node {
        ...
        using iterator =
          typename std::conditional<Direction == direction::Forward,
                                    forward_iterator, reverse_iterator>::type;

        template <bool Direction = direction::Forward>
        typename std::enable_if<
          Direction ==
            radix_tree<Key, Value, BytesView, MtMode>::node::direction::Forward,
        ...
        typename std::enable_if<
          Direction ==
            radix_tree<Key, Value, BytesView, MtMode>::node::direction::Reverse,
          typename radix_tree<Key, Value, BytesView,
      ...
      struct radix_tree<Key, Value, BytesView, MtMode>::radix_tree_iterator {
       ...
       public:
        ...
        using iterator_category =
          typename std::conditional<MtMode, std::forward_iterator_tag,
        ...
        void assign_val(
          basic_string_view<typename V::value_type, typename V::traits_type>
            rhs);
       ...
       private:
        ...
        template <typename T> void replace_val(T &&rhs);
       ...
       *
       * @throw pmem::pool_error if an object is not in persistent memory.
       * @throw pmem::transaction_alloc_error when allocating memory in
       * transaction failed.
       * @throw pmem::transaction_scope_error if constructor wasn't called in
       * transaction.
       * @throw rethrows element constructor exception.
       ...
       */
      template <typename Key, typename Value, typename BytesView, bool MtMode>
      bool radix_tree<Key, Value, BytesView, MtMode>::empty() const noexcept {
        return size_ == 0;
      }
       ...
       */
      template <typename Key, typename Value, typename BytesView, bool MtMode>
      void radix_tree<Key, Value, BytesView, MtMode>::swap(radix_tree &rhs) {
        auto pop = pool_by_vptr(this);
      ...
      /**
       * Performs full epochs synchronisation. Transactionally collects and
       * frees all garbage produced by erase, clear, insert_or_assign or
       * assign_val in concurrent mode (if MtMode == true).
       *
      ...
      void radix_tree<Key, Value, BytesView, MtMode>::garbage_collect_force() {
        ebr_->full_sync();
        for(size_t i = 0; i < EPOCHS_NUMBER; ++i) {
          clear_garbage(i);
        }
      ...
      template <typename Key, typename Value, typename BytesView, bool MtMode>
      template <bool Mt, typename Enable>
      void radix_tree<Key, Value, BytesView, MtMode>::garbage_collect() {
        ebr_->sync();
        clear_garbage(ebr_->gc_epoch());
      }

      template <typename Key, typename Value, typename BytesView, bool MtMode>
      void radix_tree<Key, Value, BytesView, MtMode>::clear_garbage(size_t n) {
          ...
          for(auto &e : garbages[n]) {
            if(is_leaf(e))
              delete_persistent<radix_tree::leaf>(
                persistent_ptr<radix_tree::leaf>(get_leaf(e)));
            else
              delete_persistent<radix_tree::node>(
                persistent_ptr<radix_tree::node>(get_node(e)));
          }
      ...
      template <typename Key, typename Value, typename BytesView, bool MtMode>
      void radix_tree<Key, Value, BytesView, MtMode>::store(
        std::atomic<detail::tagged_ptr<leaf, node>> &ptr,
        pointer_type desired) {
      ...
      /**
       * If MtMode == true, this function must be called before each application
       * close and before calling radix destructor or there will be possible a
       * memory leak.
       */
      template <typename Key, typename Value, typename BytesView, bool MtMode>
      template <bool Mt, typename Enable>
      void radix_tree<Key, Value, BytesView, MtMode>::runtime_finalize_mt() {
        if(ebr_) {
          delete ebr_;
        }
        ebr_ = nullptr;
      ...
      /**
       * Registers and returns a new worker, which can perform critical
       * operations (accessing some shared data that can be removed in other
       * threads). There can be only one worker per thread. The worker will be
       * automatically unregistered in the destructor.
       *
      ...
      radix_tree<Key, Value, BytesView, MtMode>::descend(pointer_type root_snap,
        ...
        if(n) {
          return std::pair<leaf *, path_type>{get_leaf(n), std::move(path)};
        } else {
      ...
      template <typename Key, typename Value, typename BytesView, bool MtMode>
      template <typename K1, typename K2>
      bool radix_tree<Key, Value, BytesView, MtMode>::keys_equal(const K1 &k1,
                                                                 const K2 &k2) {
      ...
      template <typename Key, typename Value, typename BytesView, bool MtMode>
      template <typename K1, typename K2>
      int radix_tree<Key, Value, BytesView, MtMode>::compare(const K1 &k1,
                                                             const K2 &k2,
                                                             byten_t offset) {
       ...
       */
      template <typename Key, typename Value, typename BytesView, bool MtMode>
      bool radix_tree<Key, Value, BytesView, MtMode>::path_length_equal(
      ...
      radix_tree<Key, Value, BytesView, MtMode>::follow_path(
        const path_type &path, byten_t diff, bitn_t sh) const {
        assert(path.size());
      ...
      template <typename Key, typename Value, typename BytesView, bool MtMode>
      template <typename K, typename F, class... Args>
      std::pair<typename radix_tree<Key, Value, BytesView, MtMode>::iterator,
                bool>
      radix_tree<Key, Value, BytesView, MtMode>::internal_emplace(
        ...
        if(!n) {
          ...
          flat_transaction::run(pop, [&] { store(*slot, make_leaf(prev)); });
        ...
        if(diff == key.size()) {
          if(!is_leaf(n) && path_length_equal(key.size(), n)) {
            ...
            flat_transaction::run(
              pop, [&] { store(n->embedded_entry, make_leaf(n)); });

            return {iterator(get_leaf(load(n->embedded_entry)), this), true};
          }
          ...
          pointer_type node;
          flat_transaction::run(pop, [&] {
            node = make_persistent<radix_tree::node>(load(parent_ref(n)), diff,
                                                     bitn_t(FIRST_NIB));
            store(node->embedded_entry, make_leaf(node));
            store(node->child[slice_index(leaf_key[diff], bitn_t(FIRST_NIB))],
                  n);
          ...
          return {iterator(get_leaf(load(node->embedded_entry)), this), true};
        }
        ...
        if(diff == leaf_key.size()) {
          ...
          flat_transaction::run(pop, [&] {
            ...
            store(node->child[slice_index(key[diff], bitn_t(FIRST_NIB))],
                  make_leaf(node));
        ...
        flat_transaction::run(pop, [&] {
          node =
            make_persistent<radix_tree::node>(load(parent_ref(n)), diff, sh);
        ...
        return {
          iterator(get_leaf(load(node->child[slice_index(key[diff], sh)])),
                   this),
          true};
      }
       ...
       * if the insertion does not happen, which makes it easy to manipulate
       * maps whose values are move-only types. In addition, try_emplace treats
       * the key and the arguments to the mapped_type separately, unlike
      ...
      radix_tree<Key, Value, BytesView, MtMode>::try_emplace(const key_type &k,
        ...
        return internal_emplace(k, [&](pointer_type parent) {
          size_++;
          return leaf::make_key_args(parent, k, std::forward<Args>(args)...);
        });
      }
      ...
      /**
       * Inserts element using move semantic if the tree doesn't already contain
       ...
       */
      template <typename Key, typename Value, typename BytesView, bool MtMode>
      void radix_tree<Key, Value, BytesView, MtMode>::insert(
        std::initializer_list<value_type> il) {
       ...
       * is, a std::pair).
       *
       * This overload only participates in overload resolution if BytesView
       * struct has a type member named is_transparent.
       *
       * @param[in] k the key used both to look up and to insert if not found.
       * @param[in] args arguments to forward to the constructor of the element.
       *
      ...
      auto
      radix_tree<Key, Value, BytesView, MtMode>::try_emplace(K &&k,
                                                             Args &&...args) ->
        typename std::enable_if<
          detail::has_is_transparent<BV>::value &&
            !std::is_same<typename std::remove_const<
       ...
       *
       * @return std::pair<iterator,bool> The bool component is true if the
       * insertion took place and false if the assignment took place. The
       * iterator component is pointing at the element that was inserted or
       * updated.
       *
       * @throw pmem::transaction_error when snapshotting failed.
       * @throw pmem::transaction_alloc_error when allocating new memory
      ...
      std::pair<typename radix_tree<Key, Value, BytesView, MtMode>::iterator,
                bool>
      radix_tree<Key, Value, BytesView, MtMode>::insert_or_assign(
       ...
       * std::forward<M>(obj) to the mapped_type corresponding to the key k. If
       * the key does not exist, inserts the new value as if by insert,
       * constructing it from value_type(std::forward<K>(k),
       * std::forward<M>(obj)).
       *
       * This overload only participates in overload resolution if BytesView
       * struct has a type member named is_transparent.
      ...
      std::pair<typename radix_tree<Key, Value, BytesView, MtMode>::iterator,
                bool>
      radix_tree<Key, Value, BytesView, MtMode>::insert_or_assign(K &&k,
                                                                  M &&obj) {
       ...
       * @param[in] k key value of the element to search for.
       *
       * @return Const iterator to an element with key equivalent to key. If no
       * such element is found, past-the-end iterator is returned.
       */
      template <typename Key, typename Value, typename BytesView, bool MtMode>
      ...
      template <typename K>
      typename radix_tree<Key, Value, BytesView, MtMode>::leaf *
      radix_tree<Key, Value, BytesView, MtMode>::internal_find(
        const K &k) const {
        auto key = bytes_view(k);
       ...
       */
      template <typename Key, typename Value, typename BytesView, bool MtMode>
      void radix_tree<Key, Value, BytesView, MtMode>::clear() {
        if(size() != 0)
          erase(begin(), end());
      }
      ...
      radix_tree<Key, Value, BytesView, MtMode>::erase(const_iterator pos) {
        ...
        flat_transaction::run(pop, [&] {
          ...
          store(const_cast<atomic_pointer_type &>(*parent->find_child(leaf)),
                nullptr);
          ...
          auto *child_slot =
            parent ? const_cast<atomic_pointer_type *>(&*parent->find_child(n))
                   : &root;
      ...
      /**
       * Deletes node/leaf pointed by ptr. If concurrent mode is used, adds
       * element to the garbage list. Otherwise, frees the element immediately.
       */
      template <typename Key, typename Value, typename BytesView, bool MtMode>
      template <typename T>
      void
      ...
      template <typename Key, typename Value, typename BytesView, bool MtMode>
      template <bool Lower, typename K>
      bool radix_tree<Key, Value, BytesView, MtMode>::validate_bound(
        const_iterator it, const K &key) const {
        if(it == cend())
          return true;
      ...
      radix_tree<Key, Value, BytesView, MtMode>::validate_path(
        ...
        for(auto i = 0ULL; i < path.size(); i++) {
          ...
          if(path[i].node && load(parent_ref(path[i].node)) != path[i].prev)
            return false;
        }
      ...
      template <bool Lower, typename K>
      typename radix_tree<Key, Value, BytesView, MtMode>::const_iterator
      radix_tree<Key, Value, BytesView, MtMode>::internal_bound(
        const K &k) const {
        auto key = bytes_view(k);
        auto pop = pool_base(pmemobj_pool_by_ptr(this));
        ...
        while(true) {
          ...
          if(!n) {
            ...
            auto target_leaf = find_leaf<node::direction::Forward>(n);
       ...
       *
       * @return Const iterator pointing to the first element that is not less
       * than key. If no such element is found, a past-the-end iterator is
       * returned.
       */
      template <typename Key, typename Value, typename BytesView, bool MtMode>
      typename radix_tree<Key, Value, BytesView, MtMode>::const_iterator
      radix_tree<Key, Value, BytesView, MtMode>::lower_bound(
       ...
       * @param[in] k key value to compare the elements to.
       *
       * @return Const iterator pointing to the first element that is greater
       * than key. If no such element is found, a past-the-end iterator is
       * returned.
       */
      template <typename Key, typename Value, typename BytesView, bool MtMode>
      typename radix_tree<Key, Value, BytesView, MtMode>::const_iterator
      radix_tree<Key, Value, BytesView, MtMode>::upper_bound(
        const key_type &k) const {
      ...
      radix_tree<Key, Value, BytesView, MtMode>::begin() {
        ...
        return iterator(
          const_cast<typename iterator::leaf_ptr>(const_begin.leaf_), this);
      }
      ...
      radix_tree<Key, Value, BytesView, MtMode>::cbegin() const {
        while(true) {
          ...
          auto leaf = find_leaf<radix_tree::node::direction::Forward>(root_ptr);
          if(leaf) {
       ...
       * Using reverse iterators in concurrent environment is not safe.
       *
       * @return reverse_iterator pointing to the theoretical element preceding
      ...
      template <typename Key, typename Value, typename BytesView, bool MtMode>
      void radix_tree<Key, Value, BytesView, MtMode>::print_rec(
        std::ostream &os, radix_tree::pointer_type n) {
        if(!is_leaf(n)) {
          os << "\"" << get_node(n) << "\""
          ...
          for(auto it = n->begin(); it != n->end(); ++it) {
            ...
            auto ch = is_leaf(c) ? (void *)get_leaf(c) : (void *)get_node(c);

            os << "\"" << get_node(n) << "\" -> \"" << ch << "\"" << std::endl;
            print_rec(os, c);
          }
        } else {
          ...
          if(parent && n == load(parent->embedded_entry)) {
            os << "{rank=same;\"" << parent << "\";\"" << get_leaf(n) << "\"}"
               << std::endl;
          }
        }
      }
      ...
      template <typename Key, typename Value, typename BytesView, bool MtMode>
      unsigned
      radix_tree<Key, Value, BytesView, MtMode>::slice_index(char b,
                                                             uint8_t bit) {
        return static_cast<unsigned>(b >> bit) & NIB;
      }

      template <typename Key, typename Value, typename BytesView, bool MtMode>
      radix_tree<Key, Value, BytesView, MtMode>::node::forward_iterator::
        forward_iterator(pointer child, const node *n)
          : child(child), n(n) {}

      template <typename Key, typename Value, typename BytesView, bool MtMode>
      typename radix_tree<Key, Value, BytesView, MtMode>::node::forward_iterator
      radix_tree<Key, Value, BytesView,
                 MtMode>::node::forward_iterator::operator++() {
      ...
      typename radix_tree<Key, Value, BytesView, MtMode>::node::forward_iterator
      radix_tree<Key, Value, BytesView,
                 MtMode>::node::forward_iterator::operator++(int) {
      ...
      typename radix_tree<Key, Value, BytesView,
                          MtMode>::node::forward_iterator::pointer
      radix_tree<Key, Value, BytesView,
                 MtMode>::node::forward_iterator::get_node() const {
        return n;
      }

      template <typename Key, typename Value, typename BytesView, bool MtMode>
      bool radix_tree<Key, Value, BytesView, MtMode>::node::forward_iterator::
      operator==(const forward_iterator &rhs) const {
      ...
      auto radix_tree<Key, Value, BytesView, MtMode>::node::find_child(
        const Ptr &n) const -> decltype(begin<Direction>()) {
      ...
      auto radix_tree<Key, Value, BytesView, MtMode>::node::make_iterator(
      ...
      typename radix_tree<Key, Value, BytesView, MtMode>::
        template radix_tree_iterator<IsConst>::reference
        radix_tree<Key, Value, BytesView,
       ...
       *
       * If reallocation happens, all other iterators to this element are
       * invalidated.
       *
      ...
      void radix_tree<Key, Value, BytesView, MtMode>::
        radix_tree_iterator<IsConst>::assign_val(
          basic_string_view<typename V::value_type, typename V::traits_type>
            rhs) {
        assert(leaf_);
        assert(tree);
      ...
      radix_tree<Key, Value, BytesView,
        ...
        flat_transaction::run(pop, [&] {
          store(*slot,
                leaf::make_key_args(load(old_leaf->parent), old_leaf->key(),
                                    std::forward<T>(rhs)));
      ...
      template <typename Key, typename Value, typename BytesView, bool MtMode>
      template <bool IsConst>
      bool radix_tree<Key, Value, BytesView,
      ...
      /*
       * If MtMode == true it's not safe to use this operator (iterator may end
       * up invalid if concurrent erase happen).
      ...
      bool radix_tree<Key, Value, BytesView,
        ...
        while(true) {
          if(!leaf_) {
            ...
            leaf_ =
              const_cast<leaf_ptr>(tree->template find_leaf<direction>(r));
      ...
      template <bool IsConst>
      template <bool C>
      bool radix_tree<Key, Value, BytesView, MtMode>::radix_tree_iterator<
      ...
      template <bool Direction, typename Iterator>
      std::pair<
        bool, const typename radix_tree<Key, Value, BytesView, MtMode>::leaf *>
      radix_tree<Key, Value, BytesView, MtMode>::next_leaf(
        Iterator node, pointer_type parent) const {
        while(true) {
          ++node;
      ...
      template <typename Key, typename Value, typename BytesView, bool MtMode>
      Key &radix_tree<Key, Value, BytesView, MtMode>::leaf::key() {
        auto &const_key = const_cast<const leaf *>(this)->key();
        return *const_cast<Key *>(&const_key);
      }

      template <typename Key, typename Value, typename BytesView, bool MtMode>
      Value &radix_tree<Key, Value, BytesView, MtMode>::leaf::value() {
        auto &const_value = const_cast<const leaf *>(this)->value();
        return *const_cast<Value *>(&const_value);
      }

      template <typename Key, typename Value, typename BytesView, bool MtMode>
      const Key &radix_tree<Key, Value, BytesView, MtMode>::leaf::key() const {
      ...
      persistent_ptr<typename radix_tree<Key, Value, BytesView, MtMode>::leaf>
      radix_tree<Key, Value, BytesView, MtMode>::leaf::make(
        pointer_type parent, detail::pair<K, V> &&p) {
        return make(parent, std::piecewise_construct,
      ...
      persistent_ptr<typename radix_tree<Key, Value, BytesView, MtMode>::leaf>
      radix_tree<Key, Value, BytesView, MtMode>::leaf::make(
        pointer_type parent, std::pair<K, V> &&p) {
        return make(parent, std::piecewise_construct,
                    std::forward_as_tuple(std::move(p.first)),
                    std::forward_as_tuple(std::move(p.second)));
      }
      ...
      persistent_ptr<typename radix_tree<Key, Value, BytesView, MtMode>::leaf>
      radix_tree<Key, Value, BytesView, MtMode>::leaf::make(
        pointer_type parent, const std::pair<K, V> &p) {
        return make(parent, std::piecewise_construct,
                    std::forward_as_tuple(p.first),
                    std::forward_as_tuple(p.second));
      }
      ...
      /**
       * Private helper function. Checks if radix tree resides on pmem and
       * throws an exception if not.
       *
       * @throw pool_error if radix tree doesn't reside on pmem.
       */
      template <typename Key, typename Value, typename BytesView, bool MtMode>
      void radix_tree<Key, Value, BytesView, MtMode>::check_pmem() {
       ...
       */
      template <typename Key, typename Value, typename BytesView, bool MtMode>
      void radix_tree<Key, Value, BytesView, MtMode>::check_tx_stage_work() {
      ...
      template <typename Key, typename Value, typename BytesView, bool MtMode>
      bool radix_tree<Key, Value, BytesView, MtMode>::is_leaf(
      ...
      struct bytes_view<T, typename std::enable_if<is_string<T>::value>::type> {
        ...
        size_t size() const { return s.size() * sizeof(CharT); }

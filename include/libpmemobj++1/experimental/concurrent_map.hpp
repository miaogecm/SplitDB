...
namespace pmem {
  namespace obj {
    namespace experimental {
       ...
       * Search, removal, and insertion operations have average logarithmic
       * complexity. Everywhere the concurrent_map uses the Compare
       * requirements, uniqueness is determined by using the equivalence
       * relation. In imprecise terms, two objects a and b are considered
       * equivalent (not unique) if neither compares less than the other:
       * !comp(a, b) && !comp(b, a).
       *
       * The implementation is based on the lock-based concurrent skip list
       * algorithm described in
       * https://www.cs.tau.ac.il/~shanir/nir-pubs-web/Papers/OPODIS2006-BA.pdf.
       * Our concurrent skip list implementation supports concurrent insertion
       * and traversal, but not concurrent erasure. The erase method is prefixed
       * with unsafe_, to indicate that there is no concurrency safety.
       *
       * Each time, the pool with concurrent_map is being opened, the
       * concurrent_map requires runtime_initialize() to be called in order to
       * restore the map state after process restart.
       *
       * Key, Value, Comp and Allcoator types should be persistent memory aware
       * types. Allocator type should satisfies the named requirements
       * (https://en.cppreference.com/w/cpp/named_req/Allocator). The allocate()
       * and deallocate() methods are called inside transactions.
      ...
      class concurrent_map
          : public detail::concurrent_skip_list<detail::map_traits<
              ...
              false, 64>> {
        using traits_type =
          detail::map_traits<Key, Value, Comp, detail::default_random_generator,
                             Allocator, false, 64>;
       ...
       public:
         ...
         *
         */
        concurrent_map &operator=(std::initializer_list<value_type> ilist) {
          return static_cast<concurrent_map &>(base_type::operator=(ilist));
        }
      };
      ...
      void swap(concurrent_map<Key, Value, Comp, Allocator> &lhs,

...
namespace pmem {
  namespace obj {
    namespace experimental {
      ...
      /**
       * Atomic specialization of a persistent ptr (self_relative_ptr) that
       *manages its persistence by itself.
       *
       * In a multi-threaded scenario, the persistence of this ptr is guaranteed
       *when it is visible to (or read by) other threads. Performance-wise, two
       *different versions of this struct are provided:
       *  - Read-optimized - data is flushed along the write operation. If more
       *reads are expected it's probably better to use this scenario.
       *  - Write-optimized - data is lazily flushed with a read operation. In
       *this approach data storing is expected to be faster, but data is
      ...
      struct atomic_persistent_aware_ptr {
       private:
        ...
        using accessor =
          pmem::detail::self_relative_accessor<std::atomic<std::ptrdiff_t>>;
       ...
       public:
         ...
         */
        atomic_persistent_aware_ptr(value_type value) : ptr() { store(value); }
       ...
       private:
        value_type mark_dirty(value_type ptr) const {
          auto dirty_ptr = reinterpret_cast<uintptr_t>(ptr.get()) | IS_DIRTY;
          return value_type{reinterpret_cast<T *>(dirty_ptr)};
        }

        value_type clear_dirty(value_type ptr) const {
          auto clear_ptr = reinterpret_cast<uintptr_t>(ptr.get()) & ~IS_DIRTY;
          return value_type{reinterpret_cast<T *>(clear_ptr)};
        }

        bool is_dirty(value_type ptr) const {

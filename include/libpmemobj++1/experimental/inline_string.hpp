...
namespace pmem {
  ...
  namespace obj {
    ...
    namespace experimental {
       ...
       *
       * @note Unlike other containers, it can be used on pmem and dram.
       * Modifiers (like assign()) can only be called if inline string is kept
       * on pmem, though.
       *
       * pmem::obj::experimental::basic_dram_inline_string serves similar
       * purpose to pmem::obj::string, but keeps the data within the same
       * allocation as inline_string itself. It means, the data is always kept
       * right after the inline_string structure.
       *
       * Creating an object of inline_string must be hence done as follows:
       * 1. Allocate memory of sizeof(inline_string) + size of the characters
       * string + sizeof('\0')
       ...
       public:
        ...
        operator=(const basic_dram_inline_string &rhs) {
          return static_cast<basic_dram_inline_string &>(this->operator=(
            static_cast<const basic_inline_string_base<CharT, Traits> &>(rhs)));
        }
      };
      ...
      class basic_inline_string
       ...
       private:
        template <typename T> T &&check_forward(T &&t) {
      ...
      /**
       * Returns const pointer to the underlying data. In contradiction to
       ...
       *			positive value if this is lexicographically
       *greater than other, negative value if this is lexicographically less
       *than other.
       */
      template <typename CharT, typename Traits>
      int basic_inline_string_base<CharT, Traits>::compare(
        basic_string_view<CharT, Traits> rhs) const noexcept {
        return basic_string_view<CharT, Traits>(data(), size()).compare(rhs);
      }

      /**
       * Returns reference to a character at position @param[in] p and snapshot
       * it if there is an active transaction. No bounds checking is performed.
       *
      ...
      /**
       * Returns slice and snapshots (if there is an active transaction)
      ...
      basic_inline_string_base<CharT, Traits>::range(size_type start,
                                                     size_type n) {
        if(start + n > size())
      ...
      /**
       * Return pointer to data at position p and if there is an active
       * transaction snapshot elements from p to p + n.
       */
      template <typename CharT, typename Traits>
      typename basic_inline_string_base<CharT, Traits>::pointer
      basic_inline_string_base<CharT, Traits>::snapshotted_data(size_t p,
                                                                size_t n) {
        assert(p + n <= size());
      ...
      basic_inline_string_base<CharT, Traits>::assign(
        ...
        obj::flat_transaction::run(pop, [&] {
          ...
          std::copy(rhs.data(), rhs.data() + static_cast<ptrdiff_t>(rhs.size()),
                    data());
          size_ = rhs.size();
      ...
      /**
       * A helper trait which calculates required memory capacity (in bytes) for
       * a type.
       *
       * All standard types require capacity equal to the sizeof of such type.
       */
      template <typename T> struct total_sizeof {
        template <typename... Args> static size_t value(const Args &...args) {
          return sizeof(T);
        }
      };
       ...
       * a type.
       *
       * Inline_string requires capacity of sizeof(basic_inline_string<CharT>) +
       ...
       * a type.
       *
       * Inline_string requires capacity of
       * sizeof(basic_dram_inline_string<CharT>) + size of the data itself.
       */
      template <typename CharT, typename Traits>
  ...
  namespace detail {
    ...
    template <typename> struct is_inline_string : std::false_type {};

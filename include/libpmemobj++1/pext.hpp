...
namespace pmem {
  ...
  namespace obj {
     ...
     * @relates p
     */
    template <typename T> std::istream &operator>>(std::istream &is, p<T> &pp) {
      is >> pp.get_rw();
      return is;
    }
     ...
     * @relates p
     */
    template <typename T> p<T> &operator++(p<T> &pp) {
      ++(pp.get_rw());
      return pp;
    }
     ...
     * @relates p
     */
    template <typename T> p<T> operator++(p<T> &pp, int) {
      p<T> temp = pp;
      ++pp;
      return temp;
    }
...
namespace std {
   ...
   */
  template <typename T> struct less<pmem::obj::p<T>> {
    size_t

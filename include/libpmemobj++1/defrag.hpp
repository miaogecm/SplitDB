...
namespace pmem {
  ...
  namespace obj {
    ...
    template <typename T>
    static constexpr
      typename std::enable_if<detail::t_is_defragmentable<T>::value, bool>::type
    ...
    class defrag {
     public:
       ...
       */
      template <typename T>
      typename std::enable_if<is_defragmentable<T>(), void>::type add(T &t) {
        if(pmemobj_pool_by_ptr(&t) != pop.handle())
          throw std::runtime_error("object is not from the chosen pool");

        t.for_each_ptr(
          [&](persistent_ptr_base &ptr) { this->container.push_back(&ptr); });
      }
       ...
       */
      template <typename T> void add(persistent_ptr<T> &ptr) {
      ...
      pobj_defrag_result run() {
        pobj_defrag_result result =
          this->pop.defrag(this->container.data(), this->container.size());

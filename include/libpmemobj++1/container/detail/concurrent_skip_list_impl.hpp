...
namespace pmem {
  namespace detail {
    ...
    inline void try_insert_node_finish_marker() {}
    ...
    inline void
    allocator_copy_assignment(MyAlloc &my_allocator,
                              OtherAlloc &other_allocator, std::true_type) {
     ...
     */
    template <typename MyAlloc, typename OtherAlloc>
    inline void allocator_copy_assignment(MyAlloc &, OtherAlloc &,
                                          std::false_type) { /* NO COPY */
    }
    ...
    template <typename MyAlloc, typename OtherAlloc>
    inline void
    allocator_move_assignment(MyAlloc &my_allocator,
                              OtherAlloc &other_allocator, std::true_type) {
     ...
     */
    template <typename MyAlloc, typename OtherAlloc>
    inline void allocator_move_assignment(MyAlloc &, OtherAlloc &,
                                          std::false_type) { /* NO MOVE */
    }
    ...
    template <typename MyAlloc, typename OtherAlloc>
    inline void
    allocator_swap(MyAlloc &, OtherAlloc &, std::false_type) { /* NO SWAP */
    }
    ...
    class skip_list_node {
     public:
      ...
      using node_pointer = obj::experimental::self_relative_ptr<skip_list_node>;
      ...
      skip_list_node(size_type levels) : height_(levels) {
        for(size_type lev = 0; lev < height_; ++lev)
          detail::create<atomic_node_pointer>(&get_next(lev), nullptr);
         ...
         */
        for(size_type lev = 0; lev < height_; ++lev) {
          VALGRIND_HG_DISABLE_CHECKING(&get_next(lev), sizeof(get_next(lev)));
        }
#endif
      }
      ...
      skip_list_node(size_type levels, const node_pointer *new_nexts)
          : height_(levels) {
        for(size_type lev = 0; lev < height_; ++lev)
          detail::create<atomic_node_pointer>(&get_next(lev), new_nexts[lev]);
      ...
      pointer get() noexcept { return &val; }

      const_pointer get() const noexcept { return &val; }
       ...
       */
      void set_next_tx(size_type level, node_pointer next) {
        assert(level < height());
      ...
      void set_next(obj::pool_base pop, size_type level, node_pointer next) {
        assert(level < height());
        auto &node = get_next(level);
        node.store(next, std::memory_order_release);
      ...
      void set_nexts(const node_pointer *new_nexts, size_type h) {
        assert(h == height());
      ...
      void set_nexts(obj::pool_base pop, const node_pointer *new_nexts,
                     size_type h) {
      ...
      lock_type acquire() { return lock_type(mutex); }

     private:
      atomic_node_pointer *get_nexts() {
        return reinterpret_cast<atomic_node_pointer *>(this + 1);
      }

      atomic_node_pointer &get_next(size_type level) {
        auto *arr = get_nexts();
        return arr[level];
      }

      const atomic_node_pointer &get_next(size_type level) const {
        auto *arr = reinterpret_cast<const atomic_node_pointer *>(this + 1);
        return arr[level];
      }
    ...
    template <typename NodeType, bool is_const> class skip_list_iterator {
     ...
     public:
      ...
      using reference =
        typename std::conditional<is_const, typename node_type::const_reference,
      ...
      reference operator*() const { return *(node->get()); }

      pointer operator->() const { return node->get(); }
    ...
    template <typename T, bool M, bool U>
    bool operator==(const skip_list_iterator<T, M> &lhs,
                    const skip_list_iterator<T, U> &rhs) {
    ...
    struct default_random_generator {
      ...
      size_t operator()() {
        static thread_local gen_type engine(static_cast<size_t>(time(0)));

        return engine();
      }

      static constexpr result_type min() { return gen_type::min(); }

      static constexpr result_type max() { return gen_type::max(); }
    };
     ...
     * concurrent_skip_list requires runtime_initialize() to be called in order
     * to restore the state after process restart.
     *
     * Traits template parameter allows to specify properties of the
     * concurrent_ski_list. The Traits type should has the following member
     * types:
     ...
     * * max_level - The constant value which specify the number of layers in
     * the skip list.
     * * random_generator_type - The type of random generator used by the skip
     * list. It should be thread-safe.
     */
    template <typename Traits> class concurrent_skip_list {
     protected:
      ...
      using random_level_generator_type =
        geometric_level_generator<typename traits_type::random_generator_type,
                                  MAX_LEVEL>;
     ...
     public:
      ...
      concurrent_skip_list(const concurrent_skip_list &other)
          : _node_allocator(
            ...
            _compare(other._compare), _rnd_generator(other._rnd_generator) {
        check_tx_stage_work();
        init();
        internal_copy(other);
      ...
      concurrent_skip_list(const concurrent_skip_list &other,
          ...
          : _node_allocator(alloc), _compare(other._compare),
      ...
      concurrent_skip_list &operator=(const concurrent_skip_list &other) {
        ...
        obj::flat_transaction::run(pop, [&] {
          ...
          clear();
          allocator_copy_assignment(_node_allocator, other._node_allocator,
                                    pocca_t());
      ...
      concurrent_skip_list &operator=(concurrent_skip_list &&other) {
        ...
        obj::flat_transaction::run(pop, [&] {
          ...
          clear();
          if(pocma_t::value || _node_allocator == other._node_allocator) {
            delete_dummy_head();
            allocator_move_assignment(_node_allocator, other._node_allocator,
                                      pocma_t());
            _compare = other._compare;
            _rnd_generator = other._rnd_generator;
            internal_move(std::move(other));
       ...
       * @throw rethrows constructor exception.
       */
      template <typename P, typename std::enable_if<std::is_constructible<
      ...
      std::pair<iterator, bool> try_emplace(key_type &&k, Args &&...args) {
        return internal_try_emplace(std::move(k), std::forward<Args>(args)...);
      }
      ...
      typename std::enable_if<has_is_transparent<key_compare>::value &&
      ...
      size_type unsafe_erase(const key_type &key) {
        ...
        size_type sz =
          static_cast<size_type>(std::distance(range.first, range.second));
      ...
      iterator find_lower_eq(const key_type &key) {
        auto it =
          internal_get_biggest_less_than(key, not_greater_compare(_compare));
        return iterator(const_cast<typename iterator::node_ptr>(it.node));
      }
      ...
      const_iterator find_lower_eq(const key_type &key) const {
        return internal_get_biggest_less_than(key,
       ...
       */
      iterator begin() { return iterator(dummy_head.get()->next(0).get()); }
      ...
      void swap(concurrent_skip_list &other) {
        ...
        obj::flat_transaction::run(pop, [&] {
          ...
          allocator_swap(_node_allocator, other._node_allocator, pocs_t());
          ...
          obj::flat_transaction::snapshot((size_t *)&_size);
          obj::flat_transaction::snapshot((size_t *)&(other._size));
     ...
     private:
      ...
      struct tls_entry_type {
        ...
        char reserved[64 - sizeof(decltype(ptr)) - sizeof(decltype(size_diff)) -
      ...
      static const_reference get_val(const_node_ptr n) {
        assert(n);
        return *(n->get());
      }

      static reference get_val(node_ptr n) {
        assert(n);
        return *(n->get());
      }

      static const key_type &get_key(const_node_ptr n) {
        assert(n);
        return traits_type::get_key(get_val(n));
      }

      template <typename K> iterator internal_find(const K &key) {
        iterator it = lower_bound(key);
        return (it == end() || _compare(key, traits_type::get_key(*it))) ? end()
                                                                         : it;
      }

      template <typename K> const_iterator internal_find(const K &key) const {
        const_iterator it = lower_bound(key);
        return (it == end() || _compare(key, traits_type::get_key(*it))) ? end()
                                                                         : it;
      }

      template <typename K> size_type internal_count(const K &key) const {
      ...
      persistent_node_ptr
      internal_find_position(size_type level, pointer_type &prev, const K &key,
       ...
       */
      template <typename K>
      void find_insert_pos(prev_array_type &prev_nodes,
      ...
      void fill_prev_next_arrays(prev_array_type &prev_nodes,
                                 next_array_type &next_nodes, const K &key,
      ...
      template <typename K, typename... Args>
      std::pair<iterator, bool> internal_try_emplace(K &&key, Args &&...args) {
      ...
      template <typename... Args>
      std::pair<iterator, bool> internal_emplace(Args &&...args) {
        check_outside_tx();
        ...
        obj::flat_transaction::run(pop, [&] {
          ...
          tls_entry.ptr = create_node(std::forward<Args>(args)...);
          ++tls_entry.size_diff;
        ...
        std::pair<iterator, bool> insert_result = internal_insert_node(
          get_key(n), height,
          [&](const next_array_type &next_nodes) -> persistent_node_ptr & {
      ...
      std::pair<iterator, bool> internal_unsafe_emplace(Args &&...args) {
        ...
        persistent_node_ptr new_node = create_node(std::forward<Args>(args)...);
      ...
      std::pair<iterator, bool> internal_insert(const K &key, Args &&...args) {
        ...
        std::pair<iterator, bool> insert_result = internal_insert_node(
          ...
          [&](const next_array_type &next_nodes) -> persistent_node_ptr & {
            ...
            tls_entry.ptr =
              create_node(std::forward_as_tuple(height, next_nodes.data()),
                          std::forward_as_tuple(std::forward<Args>(args)...));
      ...
      internal_insert_node(const K &key, size_type height,
        ...
        do {
          ...
          if(next && !allow_multimapping && !_compare(key, get_key(next))) {

            return std::pair<iterator, bool>(iterator(next), false);
          }

        } while((n = try_insert_node(
                   prev_nodes, next_nodes, height,
                   std::forward<PrepareNode>(prepare_new_node))) == nullptr);
      ...
      try_insert_node(prev_array_type &prev_nodes,
        ...
        for(size_type level = 0; level < height; ++level) {
          ...
          assert(prev_nodes[level]->next(level) == next_nodes[level]);
          assert(prev_nodes[level]->next(level) == n->next(level));
      ...
      bool check_prev_array(const prev_array_type &prevs, size_type height) {
        for(size_type l = 1; l < height; ++l) {
          ...
          assert(!_compare(get_key(prevs[l - 1]), get_key(prevs[l])));
        }

        return true;
      }

      bool try_lock_nodes(size_type height, prev_array_type &prevs,
      ...
      internal_erase(const_iterator pos, obj::p<difference_type> &size_diff) {
        ...
        std::pair<persistent_node_ptr, persistent_node_ptr> extract_result(
          nullptr, nullptr);
      ...
      internal_extract(const_iterator it) {
        ...
        if(!_compare(key, get_key(erase_node))) {
          ...
          return internal_extract_node(prev_nodes, next_nodes, erase_node);
        }

        return std::pair<persistent_node_ptr, persistent_node_ptr>(nullptr,
                                                                   nullptr);
      }
      ...
      internal_extract_node(const prev_array_type &prev_nodes,
        ...
        for(size_type level = 0; level < erase_node->height(); ++level) {
          ...
          prev_nodes[level]->set_next_tx(level, erase_node->next(level));
        }
      ...
      size_type random_level() { return _rnd_generator(); }
      ...
      persistent_node_ptr create_node(Args &&...args) {
        ...
        return create_node(std::forward_as_tuple(levels),
                           std::forward_as_tuple(std::forward<Args>(args)...));
      }

      template <typename... NodeArgs, typename... ValueArgs>
      persistent_node_ptr create_node(std::tuple<NodeArgs...> &&node_args,
                                      std::tuple<ValueArgs...> &&value_args) {
        assert(pmemobj_tx_stage() == TX_STAGE_WORK);

        persistent_node_ptr node =
          creates_dummy_node(std::forward<std::tuple<NodeArgs...>>(node_args),
                             index_sequence_for<NodeArgs...>{});

        construct_value_type(node,
      ...
      void construct_value_type(persistent_node_ptr node, Tuple &&args,
                                index_sequence<I...>) {
       ...
       */
      void create_dummy_head() { dummy_head = creates_dummy_node(MAX_LEVEL); }

      template <typename Tuple, size_t... I>
      persistent_node_ptr
      creates_dummy_node(Tuple &&args, index_sequence<I...>) {
        return creates_dummy_node(std::get<I>(std::forward<Tuple>(args))...);
      }
      ...
      persistent_node_ptr creates_dummy_node(size_type height, Args &&...args) {
        ...
        persistent_node_ptr n =
          node_allocator_traits::allocate(_node_allocator, sz).raw();

        assert(n != nullptr);

        node_allocator_traits::construct(_node_allocator, n.get(), height,
                                         std::forward<Args>(args)...);
      ...
      void delete_node(persistent_node_ptr &node) {
        assert(pmemobj_tx_stage() == TX_STAGE_WORK);
        node_ptr n = node.get();
        ...
        /* Destroy value */
        if(!is_dummy)
          node_allocator_traits::destroy(_node_allocator, n->get());
        /* Destroy node */
      ...
      void deallocate_node(persistent_node_ptr &node, size_type sz) {
        /*
         ...
         */
        obj::persistent_ptr<uint8_t> tmp = node.to_persistent_ptr().raw();
      ...
      iterator get_iterator(const_iterator it) {
        return iterator(const_cast<typename iterator::node_ptr>(it.node));
      }
      ...
      void tls_restore() {
        ...
        assert(last_run_size >= 0 ||
               on_init_size > static_cast<size_type>(std::abs(last_run_size)));
        obj::flat_transaction::run(pop, [&] {
          tls_data.clear();
      ...
      void complete_insert(tls_entry_type &tls_entry) {
        persistent_node_ptr &node = tls_entry.ptr;
        assert(node != nullptr);
    ...
    template <typename Key, typename Value, typename KeyCompare,
              typename RND_GENERATOR, typename Allocator,
              bool AllowMultimapping, size_t MAX_LEVEL>
    class map_traits {
     public:
      static constexpr size_t max_level = MAX_LEVEL;

  ...
  namespace obj {
    ...
    class basic_string {
     public:
      ...
      template <class T,
                typename Enable = typename std::enable_if<
                  std::is_convertible<const T &, pmem::obj::basic_string_view<
                                                   CharT, Traits>>::value &&
      ...
      template <class T,
                typename Enable = typename std::enable_if<std::is_convertible<
                  const T &, pmem::obj::basic_string_view<CharT, Traits>>::value>>
      ...
      basic_string &
      replace(size_type index, size_type count, const basic_string &str);
      ...
      basic_string &
      replace(size_type index, size_type count, const basic_string &str,
              size_type index2, size_type count2 = npos);
      ...
      basic_string &
      replace(const_iterator first, const_iterator last, const CharT *s);

      basic_string &
      replace(size_type index, size_type count, size_type count2, CharT ch);
     ...
     private:
      ...
      template <typename... Args> pointer replace_content(Args &&...args);

      template <typename... Args> pointer initialize(Args &&...args);
     ...
     */
    template <typename CharT, typename Traits>
    void basic_string<CharT, Traits>::for_each_ptr(for_each_ptr_function func) {
      if(!is_sso_used()) {
      ...
      if(is_sso_used()) {
        flat_transaction::run(pop, [&] {
          ...
          if(move_len > 0) {
            ...
            assert(range.end() - 1 == &sso_data()._data[index + move_len]);
          }
     ...
     * before the erase operation. If no such element exists then end() iterator
     * is returned.
     *
    ...
    template <typename CharT, typename Traits>
    typename basic_string<CharT, Traits>::iterator
    basic_string<CharT, Traits>::erase(const_iterator first,
                                       const_iterator last) {
     ...
     */
    template <typename CharT, typename Traits>
    void basic_string<CharT, Traits>::pop_back() {
      erase(size() - 1, 1);
    }
     ...
     * sso_capacity, or the smallest next power of 2, bigger than new size if it
     * is greater than old capacity, or remains the same if there is enough
     * space to store all new elements.
     *
      ...
      if(is_sso_used()) {
        ...
        flat_transaction::run(pop, [&] {
          if(new_size > sso_capacity) {
            ...
            traits_type::assign(&sso_data()._data[sz], count, ch);
     ...
     */
    template <typename CharT, typename Traits>
    void basic_string<CharT, Traits>::push_back(CharT ch) {
    ...
    template <typename CharT, typename Traits>
    basic_string<CharT, Traits> &
    basic_string<CharT, Traits>::insert(size_type index, size_type count,
                                        CharT ch) {
      if(index > size())
        throw std::out_of_range("Index out of range.");
    ...
    /**
     * Insert null-terminated C-style string pointed by s of the length
     * determined by the first null character at index transactionally.
     *
     * @param[in] index position at which the content will be inserted.
    ...
    template <typename CharT, typename Traits>
    basic_string<CharT, Traits> &
    basic_string<CharT, Traits>::insert(size_type index,
                                        const basic_string &str) {
    ...
    template <typename CharT, typename Traits>
    basic_string<CharT, Traits> &
    basic_string<CharT, Traits>::insert(size_type index1,
                                        const basic_string &str,
    ...
    basic_string<CharT, Traits>::insert(const_iterator pos, size_type count,
      ...
      flat_transaction::run(pop, [&] {
        if(is_sso_used() && new_size <= sso_capacity) {
          ...
          traits_type::assign(&sso_data()._data[index], count, ch);
          ...
          non_sso_data().insert(non_sso_data().begin() +
    ...
    /**
     * Insert characters from [first, last) range before the character pointed
     * by pos transactionally.
     *
     * @param[in] pos iterator before which the character will be inserted.
    ...
    /**
     * Insert characters from initializer list ilist before the character
     * pointed by pos transactionally.
     *
     * @param[in] pos iterator before which the character will be inserted.
    ...
    /**
     * Replace range [first, last) with the content of str string
     * transactionally.
     *
    ...
    template <typename CharT, typename Traits>
    basic_string<CharT, Traits> &
    basic_string<CharT, Traits>::replace(const_iterator first,
                                         const_iterator last,
                                         const basic_string &str) {
     ...
     * @param[in] count length of the substring that will be replaced.
     * @param[in] str that is used for the replacement.
     * @param[in] index2 start of the substring that will be used for
     * replacement.
     * @param[in] count2 length of the substring that will be used for
     * replacement.
     *
     * @return *this
     *
    ...
    basic_string<CharT, Traits>::replace(const_iterator first,
      ...
      flat_transaction::run(pop, [&] {
        if(is_sso_used() && new_size <= sso_capacity) {
          ...
          auto beg = begin() + static_cast<difference_type>(index);
      ...
      flat_transaction::run(pop, [&] {
        if(is_sso_used() && new_size <= sso_capacity) {
          ...
          traits_type::move(&sso_data()._data[index + count2],
                            &sso_data()._data[index + count],
                            sz - index - count);
          traits_type::assign(&sso_data()._data[index], count2, ch);
    ...
    /**
     * Replace range [first, last) with the characters in [s, s +
     * traits::length(s)) range transactionally.
     *
     * @param[in] first begin of the range of characters that will be replaced.
     * @param[in] last end of the range of characters that will be replaced.
     ...
     * If either count == npos or count exceeds size of *this string then
     * substring [index, size()) is used. Resulting C-style string is not
     * null-terminated.
     *
     ...
     * If npos or any value not smaller than size()-1 is passed as pos, whole
     ...
     * only includes sequences of characters that begin at or before position
     * pos, ignoring any possible match beginning after pos. If npos or any
     * value not smaller than size()-1 is passed as pos, whole string will be
     * searched.
     *
     * @param[in] s pointer to the C-style string to search for
     * @param[in] pos position at which to start the search
     ...
     * @return Position (as an Offset from the start of the string) of the first
     * character of the found substring or npos if no such substring is found.
     * If searching for an empty string returns pos unless pos > size(), in
     * which case returns size().
     */
    template <typename CharT, typename Traits>
    typename basic_string<CharT, Traits>::size_type
     ...
     *
     * @param[in] s pointer to the C-style string identifying characters to
     * search for
     * @param[in] pos position at which to start the search
    ...
    template <typename CharT, typename Traits>
    typename basic_string<CharT, Traits>::size_type
    basic_string<CharT, Traits>::find_first_of(const CharT *s,
                                               size_type pos) const {
    ...
    template <typename CharT, typename Traits>
    typename basic_string<CharT, Traits>::size_type
    basic_string<CharT, Traits>::find_first_of(CharT ch,
                                               size_type pos) const noexcept {
      return find(ch, pos);
    }
    ...
    /**
     * Finds the last character equal to any of the characters in the C-style
     * string pointed to by s. The length of the string is determined by the
     * first null character
     *
     * @param[in] s pointer to the C-style string identifying characters to
    ...
    template <typename CharT, typename Traits>
    typename basic_string<CharT, Traits>::size_type
    basic_string<CharT, Traits>::find_last_of(const CharT *s,
                                              size_type pos) const {
    ...
    template <typename CharT, typename Traits>
    typename basic_string<CharT, Traits>::size_type
    basic_string<CharT, Traits>::find_last_of(CharT ch,
                                              size_type pos) const noexcept {
      return rfind(ch, pos);
    }
    ...
    /**
     * Clears the content of a string and frees all allocated persistent memory
     ...
     */
    template <typename CharT, typename Traits>
    void basic_string<CharT, Traits>::free_data() {
      auto pop = get_pool();
    ...
    template <typename CharT, typename Traits>
    typename basic_string<CharT, Traits>::size_type
    basic_string<CharT, Traits>::get_size(size_type count,
                                          value_type ch) const {
      return count;
    }
     ...
     */
    template <typename CharT, typename Traits>
    void basic_string<CharT, Traits>::allocate(size_type n) {
      assert(pmemobj_tx_stage() == TX_STAGE_WORK);
    ...
    template <typename CharT, typename Traits>
    typename basic_string<CharT, Traits>::pointer
    basic_string<CharT, Traits>::assign_sso_data(size_type count,
                                                 value_type ch) {
      assert(pmemobj_tx_stage() == TX_STAGE_WORK);
    ...
    template <typename InputIt, typename Enable>
    typename basic_string<CharT, Traits>::pointer
    basic_string<CharT, Traits>::assign_large_data(InputIt first,
                                                   InputIt last) {
      assert(pmemobj_tx_stage() == TX_STAGE_WORK);
    ...
    template <typename CharT, typename Traits>
    typename basic_string<CharT, Traits>::pointer
    basic_string<CharT, Traits>::assign_large_data(size_type count,
                                                   value_type ch) {
      assert(pmemobj_tx_stage() == TX_STAGE_WORK);
    ...
    void basic_string<CharT, Traits>::swap(basic_string &other) {
      ...
      flat_transaction::run(pb, [&] {
        if(is_sso_used() && other.is_sso_used()) {
          ...
          std::basic_string<CharT, Traits> tmp(_short->c_str(), _short->size());
          *_short = *_long;
          *_long = tmp;
        }
      });
     ...
     */
    template <typename CharT, typename Traits>
    void basic_string<CharT, Traits>::add_sso_to_tx(size_type idx_first,
                                                    size_type num) const {
      ...
      if(num > initialized_num) {
        ...
        detail::conditional_add_to_tx(&sso_data()._data[0] + get_sso_size() + 1,
                                      num - initialized_num,
                                      POBJ_XADD_NO_SNAPSHOT);
      }
    }
     ...
     * Capacity is equal new_capacity plus sizeof(CharT) bytes for null
     * character. Content of sso string is preserved and copied to the large
     * string object.
     *
     * @pre must be called in transaction scope.
     *
     ...
     * Call basic_string &erase(size_type index, size_type count = npos) if
     * enabled.
     */
    template <typename CharT, typename Traits>
    ...
    /**
     * Participate in overload resolution only if T is not convertible to
     * size_type. Call iterator erase(const_iterator pos) if enabled.
     */
    template <typename CharT, typename Traits>
    template <typename T, typename Enable>
    typename basic_string<CharT, Traits>::iterator
    ...
    /**
     * Participate in overload resolution only if T is not convertible to
     * size_type. Call iterator insert(const_iterator pos, size_type count,
     * CharT ch) if enabled.
     */
    template <typename CharT, typename Traits>
    template <typename T, typename Enable>

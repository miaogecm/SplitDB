...
namespace pmem {
  ...
  namespace obj {
     ...
     *
     * All methods which allow write access to specific element will add it to
     ...
     *
     * @note Since C++20 pmem::obj::array cannot be initialized via constructor
     * for types with deleted copy constructor. This may change in the future.
     *
     * @note Since C++20 pmem::obj::array for const types can be initialized
     * only via default constructor. This may change in the future.
     * @ingroup containers
     */
    template <typename T, std::size_t N> struct array {

      template <typename Y, std::size_t M> struct standard_array_traits {
        using type = Y[N];
      };

      /* zero-sized array support */
      template <typename Y> struct standard_array_traits<Y, 0> {
      ...
      array &operator=(array &&other) {
        ...
        flat_transaction::run(pop, [&] {
          detail::conditional_add_to_tx(this, 1, POBJ_XADD_ASSUME_INITIALIZED);
          detail::conditional_add_to_tx(&other, 1,
                                        POBJ_XADD_ASSUME_INITIALIZED);
          std::move(other._get_data(), other._get_data() + size(), _get_data());
        });
      ...
      range(size_type start, size_type n, size_type snapshot_size) {
        ...
        return {
          range_snapshotting_iterator(_get_data() + start, _get_data() + start,
                                      n, snapshot_size),
          range_snapshotting_iterator(_get_data() + start + n,
                                      _get_data() + start, n, snapshot_size)};
      }
     ...
     private:
       ...
       * @throw pmem::pool_error if an object is not in persistent memory.
       */
      pool_base _get_pool() const { return pmem::obj::pool_by_vptr(this); }
    };
     ...
     */
    template <typename T, std::size_t N>
    typename pmem::obj::array<T, N>::iterator begin(pmem::obj::array<T, N> &a) {
      return a.begin();
    }
     ...
     */
    template <typename T, std::size_t N>
    typename pmem::obj::array<T, N>::iterator end(pmem::obj::array<T, N> &a) {
      return a.end();
    }
    ...
    inline void swap(pmem::obj::array<T, N> &lhs, pmem::obj::array<T, N> &rhs) {
      lhs.swap(rhs);
    }

  ...
  namespace obj {
    ...
    template <typename T> class vector {
     public:
      ...
      using for_each_ptr_function = std::function<void(persistent_ptr_base &)>;
     ...
     private:
      /* helper iterator */
      template <typename P> struct single_element_iterator {
        ...
        single_element_iterator &operator++() {
          count++;
          return *this;
        }

        single_element_iterator operator++(int) {
          single_element_iterator tmp = single_element_iterator(ptr, count);
          count++;
          return tmp;
        }

        difference_type operator-(const single_element_iterator &rhs) {
          return count - rhs.count;
        }

        bool operator!=(const single_element_iterator &rhs) {
     ...
     * underlying array in transaction failed.
     * @throw pmem::transaction_scope_error if constructor wasn't called in
     * transaction.
     ...
     * @throw rethrows element constructor exception.
     */
    template <typename T> vector<T>::vector(size_type count) {
      check_pmem();
      check_tx_stage_work();
     ...
     * The first and last arguments must satisfy InputIterator requirements.
     * This overload only participates in overload resolution if InputIt
     * satisfies InputIterator, to avoid ambiguity with the overload of
     ...
     * of std::vector<T> other. This constructor is not specified by STL
     * standards.
     *
     * @param[in] other reference to the vector to be copied.
     *
     ...
     * container) transactionally. Other is in a valid but empty state
     * afterwards.
     *
     * @post size() == other.size()
     * @post capacity() == other.capacity()
    ...
    /**
     * Copy assignment operator. Replaces the contents with a copy of the
     * contents of std::vector<T> other transactionally. This method is not
     ...
     * transactionally. This overload participates in overload resolution only
     * if InputIt satisfies InputIterator. All iterators, pointers and
     * references to the elements of the container are invalidated. The
     ...
     * transactionally. All iterators, pointers and references to the elements
     * of the container are invalidated. The past-the-end iterator is also
     * invalidated.
     *
     ...
     * @throw rethrows constructor's exception.
     */
    template <typename T> void vector<T>::assign(const vector &other) {
      if(this != &other)
     ...
     * move semantics (i.e. the data in other is moved from other into this
     * container) transactionally. Other is in a valid but empty state
     * afterwards. This method is not specified by STL standards.
     *
     * @post size() == other.size()
     * @post capacity() == other.capacity()
     *
     * @throw pmem::transaction_free_error when freeing underlying array failed.
     */
    template <typename T> void vector<T>::assign(vector &&other) {
      if(this == &other)
        return;
    ...
    /**
     * Copy assignment method. Replaces the contents with a copy of the contents
     * of std::vector<T> other transactionally. This method is not specified by
     * STL standards.
     *
     * @post size() == other.size()
     * @post capacity() == max(size(), other.capacity())
     ...
     * @throw rethrows constructor's exception.
     */
    template <typename T> void vector<T>::assign(const std::vector<T> &other) {
      assign(other.cbegin(), other.cend());
    }
    ...
    typename vector<T>::reference vector<T>::at(size_type n) {
      ...
      detail::conditional_add_to_tx(&_data[static_cast<difference_type>(n)], 1,
    ...
    /**
     * Access element at specific index with bounds checking. In contradiction
     * to at(), const_at() will return const_reference not depending on the
    ...
    template <typename T> typename vector<T>::reference vector<T>::front() {
      detail::conditional_add_to_tx(&_data[0], 1, POBJ_XADD_ASSUME_INITIALIZED);
     ...
     * return const_reference not depending on the const-qualification of the
     * object it is called on. std::vector doesn't provide cfront() method.
     *
     * @return reference to first element in underlying array.
     */
    template <typename T>
     ...
     * const_reference not depending on the const-qualification of the object it
     * is called on. std::vector doesn't provide cback() method.
     *
     * @return const_reference to the last element in underlying array.
     */
    template <typename T>
    ...
    /**
     * Returns const raw pointer to the underlying data. In contradiction to
     * data(), cdata() will return const_pointer not depending on the
     * const-qualification of the object it is called on. std::vector doesn't
     * provide cdata() method.
     *
     * @return const_pointer to the underlying data.
     */
    template <typename T>
    ...
    /**
     * Returns a const iterator to the end. In contradiction to end(), cend()
     * will return const_iterator not depending on the const-qualification of
     ...
     * rbegin(), crbegin() will return const_reverse_iterator not depending on
     * the const-qualification of the object it is called on.
     *
    ...
    /**
     * Returns slice and snapshots requested range. This method is not specified
     * by STL standards.
     *
     ...
     *
     * @throw std::out_of_range if any element of the range would be outside of
     * the vector.
     ...
     * @param[in] start start index of requested range.
     * @param[in] n number of elements in range.
     * @param[in] snapshot_size number of elements which should be snapshotted
     * in a bulk while traversing this slice. If provided value is larger or
     * equal to n, entire range is added to a transaction. If value is equal to
     * 0 no snapshotting happens.
     *
     * @return pmem::obj::slice from start to start + n.
     *
     * @throw std::out_of_range if any element of the range would be outside of
    ...
    vector<T>::range(size_type start, size_type n, size_type snapshot_size) {
      ...
      return {
        range_snapshotting_iterator(_data.get() + start, _data.get() + start, n,
                                    snapshot_size),
    ...
    /**
     * @return maximum number of elements the container is able to hold due to
     * PMDK limitations.
     */
    template <typename T>
     ...
     * allocated, otherwise the method does nothing. If capacity_new is greater
     * than capacity(), all iterators, including the past-the-end iterator, and
     * all references to the elements are invalidated. Otherwise, no iterators
     ...
     * failed.
     */
    template <typename T> void vector<T>::reserve(size_type capacity_new) {
    ...
    /**
     * Requests transactional removal of unused capacity. New capacity will be
     * set to current vector size. If reallocation occurs, all iterators,
     * including the past the end iterator, and all references to the elements
     * are invalidated. If no reallocation takes place, no iterators or
    ...
    /**
     * Clears the content of a vector and frees all allocated persistent memory
     * for data transactionally.
     *
     * @post size() == 0
     * @post capacity() == 0
     * @post data() == nullptr
     ...
     * @throw pmem::transaction_free_error when freeing underlying array failed.
     */
    template <typename T> void vector<T>::free_data() {
      if(_data == nullptr)
        return;
     ...
     * reallocation if the new size() is greater than the old capacity(). If the
     * new size() is greater than capacity(), all iterators and references are
     * invalidated. Otherwise, only the iterators and references before the
     * insertion point remain valid. The past-the-end iterator is also
     * invalidated.
     *
     * @param[in] pos iterator before which the content will be inserted. pos
     ...
     * CopyInsertable.
     *
     * @post capacity() is equal to the smallest next power of 2, bigger than
     * old capacity, or remains the same if there is enough space to add single
     * element.
     *
     * @throw pmem::transaction_error when snapshotting failed.
     * @throw rethrows constructor's exception.
     ...
     * transactionally. Causes reallocation if the new size() is greater than
     * the old capacity(). If the new size() is greater than capacity(), all
     ...
     *
     * @post capacity() is equal to the smallest next power of 2, bigger than
     * old capacity + count, or remains the same if there is enough space to add
     * count elements.
     *
     * @throw pmem::transaction_error when snapshotting failed.
     * @throw rethrows constructor's exception.
    ...
    template <typename T>
    typename vector<T>::iterator
    vector<T>::insert(const_iterator pos, size_type count,
      ...
      flat_transaction::run(pb, [&] {
        internal_insert(idx, single_element_iterator<value_type>(&value, 0),
     ...
     * iterators and references are invalidated. Otherwise, only the iterators
     * and references before the insertion point remain valid. The past-the-end
     * iterator is also invalidated. This overload participates in overload
     * resolution only if InputIt qualifies as InputIterator, to avoid ambiguity
     * with the pos-count-value overload. The behavior is undefined if first and
     * last are iterators into *this.
     *
     * @param[in] pos iterator before which the content will be inserted. pos
     * may be the end() iterator.
     ...
     *
     * @post capacity() is equal to the smallest next power of 2, bigger than
     * old capacity + std::distance(first, last), or remains the same if there
     * is enough space to add std::distance(first, last) elements.
     *
     * @throw pmem::transaction_error when snapshotting failed.
     * @throw rethrows constructor's exception.
     * @throw rethrows destructor exception.
     ...
     *
     * @post capacity() is equal to the smallest next power of 2, bigger than
     * old capacity + std::distance(ilist.begin(), ilist.end()), or remains the
     * same if there is enough space to add std::distance(ilist.begin(),
     * ilist.end()) elements.
     *
     * @throw pmem::transaction_error when snapshotting failed.
     * @throw rethrows constructor's exception.
    ...
    template <typename T>
    typename vector<T>::iterator
    vector<T>::insert(const_iterator pos,
                      std::initializer_list<value_type> ilist) {
      return insert(pos, ilist.begin(), ilist.end());
    }

    /**
     * Inserts a new element into the container directly before pos. The element
     * is constructed in-place. The arguments args... are forwarded to the
     * constructor as std::forward<Args>(args).... If the new size() is greater
     * than capacity(), all iterators and references are invalidated. Otherwise,
     * only the iterators and references before the insertion point remain
     * valid. The past-the-end iterator is also invalidated. Note that standard
     * allows args to be a self reference and internal emplace implementation
     * handles this case by creating temporary element_type object. This object
     * is being stored either on stack or on pmem, see pmem::detail::temp_value
     * for details.
     *
     ...
     * constructed in-place. The arguments args... are forwarded to the
     * constructor as std::forward<Args>(args).... If the new size() is greater
     * than capacity() then all iterators and references (including the
    ...
    /**
     * Removes the elements in the range [first, last). Invalidates iterators
     ...
     *
     * @return Iterator following the last removed element. If the iterator pos
     * refers to the last element, the end() iterator is returned. If first and
     * last refer to the same element, iterator to this element is returned.
     *
     * @pre value_type must meet the requirements of MoveAssignable.
     *
    ...
    vector<T>::erase(const_iterator first, const_iterator last) {
      ...
      flat_transaction::run(pb, [&] {
        if(!std::is_trivially_destructible<T>::value || idx + count < _size)
     ...
     * pop_back on an empty container does nothing. No iterators or references
     * except for back() and end() are invalidated.
     *
    ...
    /**
     * Resizes the container to count elements transactionally. If the current
     * size is greater than count, the container is reduced to its first count
     ...
     * count elements. If the current size is less than count, additional copies
     * of value are appended.
     *
     * @param[in] count new size of the container.
     ...
     */
    template <typename T>
    void vector<T>::for_each_ptr(for_each_ptr_function func) {
      func(_data);
    }

    /**
     * Private helper function. Must be called during transaction. Allocates
     * memory for given number of elements.
     *
     ...
     * underlying array in transaction failed.
     */
    template <typename T> void vector<T>::alloc(size_type capacity_new) {
      ...
      if(res == nullptr) {
        ...
        if(errno == ENOMEM)
          throw detail::exception_with_errormsg<pmem::transaction_out_of_memory>(
            msg);
        else
          throw detail::exception_with_errormsg<pmem::transaction_alloc_error>(
            msg);
      }
      _data = res;
    }
     ...
     */
    template <typename T> void vector<T>::check_pmem() {
     ...
     */
    template <typename T> void vector<T>::check_tx_stage_work() {
    ...
    template <typename T>
    template <typename... Args>
    void vector<T>::construct_at_end(size_type count, Args &&...args) {
      assert(pmemobj_tx_stage() == TX_STAGE_WORK);
     ...
     * participates in overload resolution only if InputIt satisfies
     * InputIterator.
     *
     * @param[in] first first iterator.
     * @param[in] last last iterator.
     *
     * @pre must be called in transaction scope.
     * @pre if initialized, range [end(), end() + std::distance(first, last))
     * must be snapshotted in current transaction.
    ...
    template <typename InputIt,
              typename std::enable_if<detail::is_input_iterator<InputIt>::value,
                                      InputIt>::type *>
    void vector<T>::construct_at_end(InputIt first, InputIt last) {
      assert(pmemobj_tx_stage() == TX_STAGE_WORK);
     ...
     * failed.
     */
    template <typename T> void vector<T>::dealloc() {
      assert(pmemobj_tx_stage() == TX_STAGE_WORK);

      if(_data != nullptr) {
        shrink(0);
        if(pmemobj_tx_free(*_data.raw_ptr()) != 0)
          throw detail::exception_with_errormsg<pmem::transaction_free_error>(
     ...
     */
    template <typename T> pool_base vector<T>::get_pool() const {
     ...
     */
    template <typename T>
    void vector<T>::move_elements_backward(pointer first, pointer last,
                                           pointer d_last) {
    ...
    vector<T>::construct_or_assign(size_type idx, InputIt first, InputIt last) {
      ...
      if(dest < cend())
        dest = std::copy_n(first, (std::min)(initialized_slots, count), dest);
    ...
    /**
     * Private helper function. Must be called during transaction. Inserts a gap
     * for count elements starting at index idx. If there is not enough space
     * available, reallocation occurs with new recommended size. Range specified
     * by first, last is then inserted into the gap.
     *
     ...
     * @pre must be called in transaction scope.
     *
     * @post if there is not enough space for additional gap, capacity changes
     * to get_recommended_capacity().
     *
     * @throw pmem::transaction_error when snapshotting failed.
     * @throw rethrows constructor's exception.
     * @throw rethrows destructor exception.
    ...
    vector<T>::internal_insert(size_type idx, InputIt first, InputIt last) {
      ...
      if(_capacity >= size() + count) {
        pointer dest =
          _data.get() + static_cast<difference_type>(size() + count);
        pointer begin = _data.get() + static_cast<difference_type>(idx);
        pointer end = _data.get() + static_cast<difference_type>(size());
        ...
        pointer old_mid = _data.get() + static_cast<difference_type>(idx);
        pointer old_end = _data.get() + static_cast<difference_type>(size());
     ...
     * memory for capacity_new number of elements and copies or moves old
     * elements to new memory area. If the current size is greater than
     * capacity_new, the container is reduced to its first capacity_new
     * elements. If was never allocated behaves as an alloc call.
     *
     * @param[in] capacity_new new capacity.
     *
     * @pre must be called in transaction scope.
     ...
     * failed.
     */
    template <typename T> void vector<T>::realloc(size_type capacity_new) {
      assert(pmemobj_tx_stage() == TX_STAGE_WORK);
    ...
    /**
     * Private helper function. Returns recommended capacity for at least
     * at_least elements.
     *
     ...
     * @throw rethrows destructor exception.
     */
    template <typename T> void vector<T>::shrink(size_type size_new) {
      assert(pmemobj_tx_stage() == TX_STAGE_WORK);
     ...
     */
    template <typename T>
    void vector<T>::add_data_to_tx(size_type idx_first, size_type num) {
      ...
      if(num > initialized_num) {
        ...
        detail::conditional_add_to_tx(
          _data.get() + size(), num - initialized_num, POBJ_XADD_NO_SNAPSHOT);
      }
    }
     ...
     *
     * Checks if containers have the same number of elements and each element in
     * lhs is equal to element in rhs at the same position.
     *
     ...
     * @param[in] rhs second vector of type pmem::obj::vector<T>
     *
     * @return true if contents of lhs are lexicographically less than contents

...
namespace std {
   ...
   */
  template <typename T> struct hash<pmem::obj::p<T>> {
...
namespace pmem {
  namespace obj {
    ...
    namespace concurrent_hash_map_internal {
      template <typename SharedMutexT> class shared_mutex_scoped_lock {
      ...
      class scoped_lock_traits {
       public:
        ...
        static bool initial_rw_state(bool write) {
          /* For upgradeable locks, initial state is always read */
          return false;
        }

        static bool upgrade_to_writer(scope_lock_type &lock) {
          return lock.upgrade_to_writer();
        }

        static bool downgrade_to_reader(scope_lock_type &lock) {
    ...
    template <typename Key, typename T, typename Hash = std::hash<Key>,
              ...
              typename ScopedLockType =
                concurrent_hash_map_internal::shared_mutex_scoped_lock<MutexType>>
    ...
    namespace concurrent_hash_map_internal {
      ...
      template <typename Hash>
      using has_transparent_key_equal =
        detail::supports<Hash, transparent_key_equal>;
      ...
      template <typename Key, typename T, typename MutexType,
                typename ScopedLockType>
      struct hash_map_node {
        ...
        using node_ptr_t =
          detail::persistent_pool_ptr<hash_map_node<Key, T, mutex_t, scoped_t>>;
       ...
       * used by hash map.
       */
      template <typename Bucket> class segment_traits {
       public:
        /** segment index type */
          ...
          if(my_seg < first_block) {
            ...
            for(segment_index_t b = blocks.first; b < blocks.second; ++b) {
              if((*my_table)[b] != nullptr) {
                delete_persistent<bucket_type[]>((*my_table)[b], block_size(b));
                (*my_table)[b] = nullptr;
              }
            }
          }
      ...
      class hash_map_base {
       public:
        ...
        static constexpr features header_features() {
          return {FEATURE_CONSISTENT_SIZE, 0};
        }

        const std::atomic<hashcode_type> &mask() const noexcept {
          return my_mask;
        }

        std::atomic<hashcode_type> &mask() noexcept { return my_mask; }

        size_t size() const { return my_size.load(std::memory_order_relaxed); }

        p<int64_t> &thread_size_diff() {
        ...
        void free_tls() {
          ...
          if((layout_features.compat & FEATURE_CONSISTENT_SIZE) && tls_ptr) {
            flat_transaction::run(pop, [&] {
         ...
         */
        bucket *get_bucket(hashcode_type h) const {
         ...
         * Check for mask race
         */
        inline bool check_mask_race(hashcode_type h, hashcode_type &m) const {
         ...
         */
        bool check_rehashing_collision(hashcode_type h, hashcode_type m_old,
        ...
        insert_new_node(bucket *b, detail::persistent_pool_ptr<Node> &new_node,
          ...
          if(pmemobj_tx_stage() == TX_STAGE_WORK) {
            insert_new_node_internal(b, new_node, std::forward<Args>(args)...);
            this->on_init_size++;
          } else {
         ...
         */
        bool check_growth(hashcode_type m, size_type sz) {
          if(sz >= m) {
            segment_index_t new_seg = static_cast<segment_index_t>(
              detail::Log2(m + 1)); /* optimized segment_index_of */
        ...
        /**
         * AllocateRootOfKvList enough segments for number of buckets
         */
         ...
         */
        void internal_swap(hash_map_base<Key, T, mutex_t, scoped_t> &table) {
          pool_base p = get_pool_base();
          {
            ...
            this->my_size =
              table.my_size.exchange(this->my_size, std::memory_order_relaxed);
       ...
       * @ingroup containers
       */
      template <typename Container, bool is_const> class hash_map_iterator {
       public:
       ...
       public: /* workaround */
        ...
        hash_map_iterator(map_ptr map, size_t index)
            : my_map(map), my_index(index), my_bucket(nullptr),
              my_node(nullptr) {
       ...
       public:
        ...
        hash_map_iterator(const hash_map_iterator &other)
            : my_map(other.my_map), my_index(other.my_index),
              my_bucket(other.my_bucket), my_node(other.my_node) {}
     ...
     * In addition to concurrent find, insert, and erase operations, the
     * algorithm employs resizing and on-demand per-bucket rehashing. The hash
     * table consists of an array of buckets, and each bucket consists of a list
     * of nodes and a read-write lock to control concurrent access by multiple
     * threads.
     *
     ...
     * concurrent_hash_map requires runtime_initialize() to be called (in order
     * to recalculate mask and restore the size).
     *
     ...
     * for owning a mutex. It should implement following methods and
     * constructors: ScopedLockType() ScopedLockType(rw_mutex_type &m, bool
     * write = true) void acquire(rw_mutex_type &m, bool write) void release()
     ...
     * report lock ordering errors. This might happen when calling find, insert
     * or erase while already holding an accessor to some element.
     *
     * The typical usage example would be:
     * @snippet concurrent_hash_map/concurrent_hash_map.cpp
     * concurrent_hash_map_ex
     *
     * The example of storing strings without necessity of using transactions
     * would be:
     * @snippet concurrent_hash_map/concurrent_hash_map_string.cpp
     * cmap_string_ex
     * @ingroup containers
     */
    template <typename Key, typename T, typename Hash, typename KeyEqual,
              typename MutexType, typename ScopedLockType>
    class concurrent_hash_map
        : protected concurrent_hash_map_internal::hash_map_base<
            Key, T, MutexType, ScopedLockType> {
     ...
     public:
      ...
      using iterator =
        concurrent_hash_map_internal::hash_map_iterator<concurrent_hash_map,
                                                        false>;
      ...
      using hasher = Hash;
      using key_equal =
        typename concurrent_hash_map_internal::key_equal_type<Hash,
                                                              KeyEqual>::type;
     ...
     protected:
       ...
       */
      using hash_map_base =
        concurrent_hash_map_internal::hash_map_base<Key, T, mutex_t, scoped_t>;
      ...
      persistent_node_ptr_t search_bucket(const K &key, bucket *b) const {
        ...
        while(n && !key_equal{}(key, n.get(this->my_pool_uuid)->item.first)) {
      ...
      class bucket_accessor : public bucket_lock_type {
       ...
       public:
        bucket_accessor(bucket_accessor &&b) noexcept : my_b(b.my_b) {
          ...
          bucket_lock_type::is_writer = b.bucket_lock_type::is_writer;
          b.my_b = nullptr;
        ...
        bucket_accessor &operator=(const bucket_accessor &other) = delete;
         ...
         */
        bool is_writer() const { return bucket_lock_type::is_writer; }
      ...
      class serial_bucket_accessor {
       ...
       public:
        serial_bucket_accessor(concurrent_hash_map *base, const hashcode_type h,
                               bool writer = false) {
          acquire(base, h, writer);
        }
      ...
      hashcode_type get_hash_code(node_ptr_t &n) {
        return hasher{}(detail::static_persistent_pool_pointer_cast<node>(n)(
                          this->my_pool_uuid)
                          ->item.first);
      }
      ...
      void rehash_bucket(bucket *b_new, const hashcode_type h) {
        ...
        using scoped_lock_traits_type =
          concurrent_hash_map_internal::scoped_lock_traits<accessor_type>;
        ...
        restart:
          for(node_ptr_t *p_old = &(b_old->node_list), n = *p_old; n;
            ...
            if((c & mask) == h) {
              if(!b_old.is_writer() &&
                 !scoped_lock_traits_type::upgrade_to_writer(b_old)) {
                goto restart;
      ...
      void check_incompat_features() {
        if(layout_features.incompat != header_features().incompat)
          throw pmem::layout_error(
            "Incompat flags mismatch, for more details go "
            "to: https://pmem.io/libpmemobj-cpp\n");

        if((layout_features.compat & FEATURE_CONSISTENT_SIZE) &&
           this->value_size != sizeof(value_type))
          throw pmem::layout_error("Size of value_type is different than the "
                                   "one stored in the pool\n");
      }
     ...
     public:
      ...
      class const_accessor
        ...
        friend class concurrent_hash_map<Key, T, Hash, KeyEqual, mutex_t,
                                         scoped_t>;
       ...
       */
      concurrent_hash_map() : hash_map_base() { runtime_initialize(); }
       ...
       */
      template <typename I> concurrent_hash_map(I first, I last) {
        runtime_initialize();
      ...
      void runtime_initialize() {
        ...
        if(!(layout_features.compat & FEATURE_CONSISTENT_SIZE)) {
          ...
          flat_transaction::run(pop, [&] {
            ...
            layout_features.compat |= FEATURE_CONSISTENT_SIZE;
          });
        } else {
          assert(this->tls_ptr != nullptr);
          this->tls_restore();
      ...
      [[deprecated("runtime_initialize(bool) is now deprecated, use "
                   "runtime_initialize(void)")]] void
       ...
       */
      size_type size() const { return hash_map_base::size(); }
      ...
      template <
        ...
        typename = typename std::enable_if<
          concurrent_hash_map_internal::has_transparent_key_equal<hasher>::value,
          K>::type>
      ...
      bool insert(const_accessor &result, value_type &&value) {
        ...
        return internal_insert(value.first, &result, false, std::move(value));
      }
      ...
      bool insert(accessor &result, value_type &&value) {
        ...
        return internal_insert(value.first, &result, true, std::move(value));
      }
      ...
      bool insert(value_type &&value) {
        ...
        return internal_insert(value.first, nullptr, false, std::move(value));
      }
      ...
      bool insert_or_assign(const key_type &key, M &&obj) {
        ...
        accessor acc;
        auto result =
          internal_insert(key, &acc, true, key, std::forward<M>(obj));
       ...
       * @throw pmem::transaction_scope_error if called inside transaction
       */
      template <typename M> bool insert_or_assign(key_type &&key, M &&obj) {
        concurrent_hash_map_internal::check_outside_tx();
      ...
      bool insert_or_assign(K &&key, M &&obj) {
        ...
        accessor acc;
        auto result = internal_insert(key, &acc, true, std::forward<K>(key),
                                      std::forward<M>(obj));
      ...
      defragment(double start_percent = 0, double amount_percent = 100) {
        ...
        if(start_percent < 0 || start_percent >= 100 || end_percent < 0 ||
           end_percent > 100 || start_percent >= end_percent) {
        ...
        size_t end_index = static_cast<size_t>((end_percent * max_index) / 100);
     ...
     protected:
       ...
       */
      bool
      try_acquire_item(const_accessor *result, node_mutex_t &mutex, bool write);
      ...
      class mutex_vector {
       public:
        ...
        bucket *push_and_try_lock(concurrent_hash_map *base, hashcode_type h) {
          ...
          auto node_ptr =
            static_cast<node *>(b->node_list.get(base->my_pool_uuid));

          while(node_ptr) {
            const_accessor ca;
            if(!base->try_acquire_item(&ca, node_ptr->mutex,
                                       /*write=*/true)) {
              vec.pop_back();
              return nullptr;
            }
            ...
            node_ptr =
              static_cast<node *>(node_ptr->next.get((base->my_pool_uuid)));
          }
        ...
        if(!n) {
          if(Bucket_rw_lock && !b.is_writer() &&
            ...
            if(n) {
              ...
              scoped_lock_traits_type::downgrade_to_reader(b);
              return n;
            }
          }
        }
      ...
      template <typename K> bool internal_erase(const K &key);
      ...
      void defrag_save_nodes(bucket *b, pmem::obj::defrag &defrag) {
        auto node_ptr =
          static_cast<node *>(b->node_list.get(this->my_pool_uuid));
        ...
        while(node_ptr) {
          ...
          node_ptr =
            static_cast<node *>(node_ptr->next.get((this->my_pool_uuid)));
        }
      }
    }; // class concurrent_hash_map
    ...
    concurrent_hash_map<Key, T, Hash, KeyEqual, MutexType,
      ...
      while(true) {
        ...
        if(!result ||
           try_acquire_item(result, node.get(this->my_pool_uuid)->mutex, write))
          break;
    ...
    concurrent_hash_map<Key, T, Hash, KeyEqual, MutexType,
      ...
      while(true) {
        ...
        if(!node) {
          ...
          new_size =
            insert_new_node(b.get(), node, std::forward<Args>(args)...);
          inserted = true;
        }
    ...
    template <typename Key, typename T, typename Hash, typename KeyEqual,
              typename MutexType, typename ScopedLockType>
    template <typename K>
    bool concurrent_hash_map<Key, T, Hash, KeyEqual, MutexType,
    ...
    template <typename Key, typename T, typename Hash, typename KeyEqual,
              typename MutexType, typename ScopedLockType>
    void concurrent_hash_map<Key, T, Hash, KeyEqual, MutexType,
                             ScopedLockType>::rehash(size_type sz) {
      concurrent_hash_map_internal::check_outside_tx();
      ...
      for(; b <= m; ++b) {
        ...
        concurrent_hash_map_internal::assert_not_locked<mutex_t, scoped_t>(
          bp->mutex);
    ...
    void concurrent_hash_map<Key, T, Hash, KeyEqual, MutexType,
      ...
      for(segment_index_t i = 0; i < sz; ++i) {
        for(node_ptr_t n = segment[i].node_list; n; n = segment[i].node_list) {
    ...
    operator==(const concurrent_hash_map<Key, T, Hash, KeyEqual, MutexType,
      ...
      typename concurrent_hash_map<Key, T, Hash, KeyEqual, MutexType,
                                   ScopedLockType>::const_iterator i(a.begin()),
        i_end(a.end());
    ...
    template <typename Key, typename T, typename Hash, typename KeyEqual,
              typename MutexType, typename ScopedLockType>
    inline void swap(
      concurrent_hash_map<Key, T, Hash, KeyEqual, MutexType, ScopedLockType> &a,

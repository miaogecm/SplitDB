...
namespace pmem {
  namespace obj {
    ...
    namespace segment_vector_internal {
       ...
       * iterators.
       */
      template <typename Container, bool is_const> class segment_iterator {
       public:
        /* Traits */
        ...
        using pointer =
          typename std::conditional<is_const, typename table_type::const_pointer,
      ...
      template <typename Container, bool is_const>
      template <bool C>
      bool segment_iterator<Container, is_const>::operator==(
        const segment_iterator<Container, C> &rhs) const {
    ...
    using fixed_size_vector_policy =
      segment_vector_internal::fixed_size_policy<pmem::obj::vector, SegmentType,
                                                 SegmentSize>;
     ...
     *
     * @pre if SegmentType for policy is specified it must contain such
     * functions as: default constructor, destructor, assign, operator[],
     * free_data, emplace_back, clear, resize, reserve, erase, capacity(),
     * size(). They must have signature the same as in vector. Also must support
     * iterators.
     *
    ...
    class segment_vector {
     public:
      ...
      using iterator =
        segment_vector_internal::segment_iterator<segment_vector, false>;
    ...
    template <typename InputIt,
              typename std::enable_if<detail::is_input_iterator<InputIt>::value,
                                      InputIt>::type *>
    void segment_vector<T, Policy>::assign(InputIt first, InputIt last) {
     ...
     */
    template <typename T, typename Policy>
    void segment_vector<T, Policy>::assign(std::initializer_list<T> ilist) {
      assign(ilist.begin(), ilist.end());
    }
     ...
     */
    template <typename T, typename Policy>
    void segment_vector<T, Policy>::assign(const segment_vector &other) {
      if(this != &other)
     ...
     */
    template <typename T, typename Policy>
    void segment_vector<T, Policy>::assign(segment_vector &&other) {
      if(this == &other)
        return;
     ...
     */
    template <typename T, typename Policy>
    void segment_vector<T, Policy>::assign(const std::vector<T> &other) {
      assign(other.cbegin(), other.cend());
    }
    ...
    segment_vector<T, Policy>::operator[](size_type n) {
      ...
      detail::conditional_add_to_tx(&element, 1, POBJ_XADD_ASSUME_INITIALIZED);
     ...
     */
    template <typename T, typename Policy>
    constexpr bool segment_vector<T, Policy>::empty() const noexcept {
      return size() == 0;
    }
     ...
     */
    template <typename T, typename Policy>
    void segment_vector<T, Policy>::reserve(size_type capacity_new) {
     ...
     */
    template <typename T, typename Policy>
    void segment_vector<T, Policy>::shrink_to_fit() {
      if(empty())
        return;
     ...
     */
    template <typename T, typename Policy>
    void segment_vector<T, Policy>::clear() {
      pool_base pb = get_pool();
     ...
     */
    template <typename T, typename Policy>
    void segment_vector<T, Policy>::free_data() {
      pool_base pb = get_pool();
      flat_transaction::run(pb, [&] {
        for(size_type i = 0; i < _segments_used; ++i)
          _data[i].free_data();
    ...
    template <typename T, typename Policy>
    typename segment_vector<T, Policy>::iterator
    segment_vector<T, Policy>::erase(const_iterator first,
                                     const_iterator last) {
      size_type count = static_cast<size_type>(std::distance(first, last));
     ...
     */
    template <typename T, typename Policy>
    void segment_vector<T, Policy>::push_back(const T &value) {
      emplace_back(value);
    }
     ...
     */
    template <typename T, typename Policy>
    void segment_vector<T, Policy>::push_back(T &&value) {
     ...
     */
    template <typename T, typename Policy>
    void segment_vector<T, Policy>::pop_back() {
      if(empty())
        return;
     ...
     */
    template <typename T, typename Policy>
    void segment_vector<T, Policy>::resize(size_type count) {
      pool_base pb = get_pool();
      flat_transaction::run(pb, [&] {
        size_type _size = size();
        if(count < _size)
     ...
     */
    template <typename T, typename Policy>
    void segment_vector<T, Policy>::swap(segment_vector &other) {
      pool_base pb = get_pool();
      flat_transaction::run(pb, [&] {
     ...
     */
    template <typename T, typename Policy>
    void segment_vector<T, Policy>::internal_reserve(size_type new_capacity) {
    ...
    template <typename T, typename Policy>
    template <typename... Args>
    void segment_vector<T, Policy>::construct(size_type idx, size_type count,
                                              Args &&...args) {
      assert(pmemobj_tx_stage() == TX_STAGE_WORK);
     ...
     */
    template <typename T, typename Policy>
    void segment_vector<T, Policy>::insert_gap(size_type idx, size_type count) {
      assert(pmemobj_tx_stage() == TX_STAGE_WORK);
      if(count == 0)
        return;
     ...
     */
    template <typename T, typename Policy>
    void segment_vector<T, Policy>::shrink(size_type size_new) {
      assert(pmemobj_tx_stage() == TX_STAGE_WORK);
     ...
     */
    template <typename T, typename Policy>
    pool_base segment_vector<T, Policy>::get_pool() const {
     ...
     */
    template <typename T, typename Policy>
    bool segment_vector<T, Policy>::segment_capacity_validation() const {
      for(size_type i = 0; i < _segments_used; ++i)

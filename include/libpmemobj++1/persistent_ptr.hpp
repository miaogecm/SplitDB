...
namespace pmem {
  ...
  namespace obj {
     ...
     */
    template <> class persistent_ptr<void> : public persistent_ptr_base {
     public:
     ...
     */
    template <> class persistent_ptr<const void> : public persistent_ptr_base {
     public:
     ...
     *
     * Casting to persistent_ptr_base can be easily done from any
     * persistent_ptr<T> objects, but when casting between convertible objects
     * be advised to use constructors or operator= specified for such
     * conversion, see:
     ...
     * casting to the second (convertible) type, the Offset will not be
     * re-calculated.
     *
     * Below you can find an example how to and how NOT to cast
     * persistent_ptr's:
     * @snippet persistent/persistent.cpp persistent_ptr_casting_example
     * @ingroup primitives
     */
    template <typename T> class persistent_ptr : public persistent_ptr_base {
     public:
      typedef persistent_ptr<T> this_type;

      template <typename U> friend class persistent_ptr;
       ...
       */
      template <class U> using rebind = pmem::obj::persistent_ptr<U>;
     ...
     protected:
       ...
       * object
       */
      template <typename U> inline ptrdiff_t calculate_offset() const {

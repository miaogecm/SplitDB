  ...
  namespace obj {
    ...
    class condition_variable {
     ...
     public:
       ...
       * #pcond fails.
       */
      template <typename Predicate> void wait(mutex &lock, Predicate pred) {
      ...
      bool
      wait_for(mutex &lock, const std::chrono::duration<Rep, Period> &rel_time,
               Predicate pred) {
        return this->wait_until_impl(lock, clock_type::now() + rel_time,
                                     std::move(pred));
      }
     ...
     private:
      ...
      void wait_impl(mutex &lock) {
        PMEMobjpool *pop = pmemobj_pool_by_ptr(this);
        if(int ret = pmemobj_cond_wait(pop, &this->pcond, lock.native_handle()))
          throw detail::exception_with_errormsg<lock_error>(
            ret, std::system_category(),
      ...
      std::cv_status wait_until_impl(
        ...
        auto ret =
          pmemobj_cond_timedwait(pop, &this->pcond, lock.native_handle(), &ts);

...
namespace pmem {
  ...
  namespace obj {
     ...
     * a transaction are made atomically with respect to persistence. It does it
     * by creating a snapshot of the variable when modified in the transaction
     * scope. The p class is not designed to be used with compound types. For
     * that see the persistent_ptr.
     ...
     * @relates p
     */
    template <class T> inline void swap(p<T> &a, p<T> &b) { a.swap(b); }

...
namespace pmem {
  ...
  namespace detail {
    /**
     * A structure that checks if it is possible to snapshot the specified
     * memory. Can have specialization.
     */
    template <typename T> struct can_do_snapshot {
     ...
     */
    template <bool is_flat> class transaction_base {
     public:
      class manual {
       public:
        /**
         ...
         */
        template <typename... L> manual(obj::pool_base &pop, L &...locks) {
          int ret = 0;

          nested = pmemobj_tx_stage() == TX_STAGE_WORK;

          if(nested) {
            ret = pmemobj_tx_begin(pop.handle(), nullptr, TX_PARAM_NONE);
          } else if(pmemobj_tx_stage() == TX_STAGE_NONE) {
            ret = pmemobj_tx_begin(pop.handle(), nullptr, TX_PARAM_CB,
          ...
          if(ret != 0)
            throw detail::exception_with_errormsg<pmem::transaction_error>(
       ...
       private:
        template <bool cnd = is_flat &&LIBPMEMOBJ_CPP_FLAT_TX_USE_FAILURE_RETURN>
        typename std::enable_if<cnd>::type set_failure_behavior() {
 ...
 */
      ...
      exec_tx(obj::pool_base &pool, std::function<void()> tx, Locks &...locks) {
        run(pool, tx, locks...);
      }
      ...
      static void snapshot(const T *addr, size_t num = 1) {
        ...
        if(pmemobj_tx_add_range_direct(addr, sizeof(*addr) * num)) {
          const char *msg = "Could not take a snapshot of given memory range.";
          if(errno == ENOMEM)
            throw detail::exception_with_errormsg<
              pmem::transaction_out_of_memory>(msg);
          else
            throw detail::exception_with_errormsg<pmem::transaction_error>(msg);
        }
      }
      ...
      static void register_callback(stage stg, std::function<void()> cb) {
        ...
        get_tx_data()->callbacks[static_cast<size_t>(stg)].push_back(cb);
      }
     ...
     private:
      ...
      using callbacks_map_type = std::array<callbacks_list_type, MAX_TX_STAGE>;

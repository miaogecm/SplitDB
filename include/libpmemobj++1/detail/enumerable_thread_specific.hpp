  ...
  namespace detail {
    ...
    inline id_manager::id_manager()
        : queue_capacity(initial_queue_capacity),
          queue(initial_queue_capacity, 0) {
     ...
     * @pre this method must be called under a lock.
     */
    inline void id_manager::release(size_t id) {
      std::unique_lock<std::mutex> lock(mutex);
     ...
     */
    inline id_manager &thread_id_type::get_id_manager() {
    ...
    inline thread_id_type::thread_id_type() {
      ...
      ANNOTATE_BENIGN_RACE_SIZED(&manager, sizeof(std::mutex),
     ...
     */
    inline thread_id_type::~thread_id_type() { get_id_manager().release(id); }

    /**
     * Obtain current thread id.
     */
    inline size_t thread_id_type::get() { return id; }
     ...
     */
    template <typename T, typename Mutex, typename Storage>
    size_t enumerable_thread_specific<T, Mutex, Storage>::get_cached_size() {
     ...
     */
    template <typename T, typename Mutex, typename Storage>
    void enumerable_thread_specific<T, Mutex, Storage>::clear() {
      auto pop = get_pool();
     ...
     */
    template <typename T, typename Mutex, typename Storage>
    bool enumerable_thread_specific<T, Mutex, Storage>::empty() const {
     ...
     *
     * @return reference to pool_base object where enumerable_thread_local
     * resides.
     */
    template <typename T, typename Mutex, typename Storage>
    obj::pool_base

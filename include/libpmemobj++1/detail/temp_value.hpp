...
#define LIBPMEMOBJ_CPP_MAX_STACK_ALLOC_SIZE 64 * (1 << 10)
    ...
    /**
     * Template class for caching objects based on constructor's variadic
     * template arguments and LIBPMEMOBJ_CPP_MAX_STACK_ALLOC_SIZE. It has two
    ...
    /*
     * Specialization for throwing constructors or objects greater than or equal
     * to LIBPMEMOBJ_CPP_MAX_STACK_ALLOC_SIZE bytes. Constructs and stores value
     * in underlying field in pmem.
     */
    template <typename T, bool NoExcept>
    struct temp_value<
      T, NoExcept,
      typename std::enable_if<
        !NoExcept || (sizeof(T) >= LIBPMEMOBJ_CPP_MAX_STACK_ALLOC_SIZE)>::type> {
      template <typename... Args> temp_value(Args &&...args) {

...
namespace pmem {
  namespace detail {
    ...
    struct tagged_ptr_impl {
      ...
      tagged_ptr_impl(std::nullptr_t) : ptr(nullptr) { assert(!(bool)*this); }
      ...
      tagged_ptr_impl &operator=(const obj::persistent_ptr<P1> &rhs) {
        ptr = add_tag(rhs.get());
      ...
      tagged_ptr_impl &operator=(const obj::persistent_ptr<P2> &rhs) {
        ptr = rhs.get();
      ...
      bool operator==(const tagged_ptr_impl &rhs) const {
      ...
      bool operator!=(const P2 *rhs) const { return !(*this == rhs); }

      void swap(tagged_ptr_impl &rhs) { ptr.swap(rhs.ptr); }
      ...
      P2 *operator->() const { return get<P2>(); }
     ...
     private:
      ...
      void *add_tag(P1 *ptr) const {
        auto tagged = reinterpret_cast<uintptr_t>(ptr) | uintptr_t(IS_TAGGED);
  ...
  struct atomic<pmem::detail::tagged_ptr<P1, P2>> {
   private:
    using ptr_type = pmem::detail::tagged_ptr_impl<
      P1, P2, atomic<pmem::obj::experimental::self_relative_ptr<void>>>;
   ...
   public:
    ...
    atomic(value_type value) : ptr() { store(value); }
    ...
    void store_with_snapshot_release(value_type desired) {
    ...
    value_type load_acquire() const noexcept {
      return load(std::memory_order_acquire);
    }

    void swap(atomic<pmem::detail::tagged_ptr<P1, P2>> &rhs) {
      auto tmp = rhs.load();

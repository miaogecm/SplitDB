...
namespace pmem {
  ...
  namespace detail {
    ...
    class volatile_state {
     public:
      template <typename T> static T *get_if_exists(const PMEMoid &oid) {
        auto &map = get_map();
      ...
      template <typename T> static T *get(const PMEMoid &oid) {
        auto &map = get_map();
...

        {
          ...
          if(it == map.end()) {
            auto ret =
              map.emplace(std::piecewise_construct, std::forward_as_tuple(oid),
            ...
            auto *user_data =
              static_cast<detail::pool_data *>(pmemobj_get_user_data(pop));

            user_data->set_cleanup(
              [oid] { clear_from_pool(oid.pool_uuid_lo); });
          }
     ...
     private:
      ...
      using value_type =
        std::unique_ptr<void, std::add_pointer<void(const void *)>::type>;

      using map_type =
        std::unordered_map<key_type, value_type, pmemoid_hash, pmemoid_equal_to>;

      using rwlock_type = std::shared_timed_mutex;

      static void clear_from_pool(uint64_t pool_id) {
        std::unique_lock<rwlock_type> lock(get_rwlock());
        auto &map = get_map();

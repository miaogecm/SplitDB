...
namespace pmem {
  ...
  namespace detail {
     ...
     */
    template <typename T> struct if_not_array {
      typedef T type;
    };
     ...
     * Template for checking if T is not an array.
     */
    template <typename T> struct if_not_array<T[]>;

    /*
     * Template for checking if T is not an array.
     */
    template <typename T, size_t N> struct if_not_array<T[N]>;

    /*
     * Template for checking if T is an array.
     */
    template <typename T> struct if_size_array;

    /*
     * Template for checking if T is an array.
     */
    template <typename T> struct if_size_array<T[]>;

    /*
     * Template for checking if T is an array.
     */
    template <typename T, size_t N> struct if_size_array<T[N]> {
      typedef T type[N];
    };
    ...
    /*
     * C-style function which calls T constructor with arguments packed in a
     * tuple.
     *
     ...
     */
    template <typename T> void destroy(typename if_size_array<T>::type &arg) {
      typedef typename detail::pp_array_type<T>::type I;
      enum { N = pp_array_elems<T>::elems };

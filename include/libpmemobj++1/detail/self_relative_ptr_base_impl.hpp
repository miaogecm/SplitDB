...
namespace pmem {
  ...
  namespace detail {
     ...
     *
     * It can be used e.g. as a parameter, where self_relative_ptr of any
     * template type is required. It is similar to self_relative_ptr<void> (it
     * can point to whatever type), but it can be used when you want to have
     * pointer to some unspecified self_relative_ptr (with
     * self_relative_ptr<void> it can't be done, because: self_relative_ptr<T>*
     * does not convert to self_relative_ptr<void>*).
     *
     * @includedoc shared/self_relative_pointer_implementation.txt
     */
    template <typename OffsetType> class self_relative_ptr_base_impl {
     public:
       ...
       */
      explicit operator void *() const noexcept { return to_void_pointer(); }
       ...
       */
      inline bool is_null() const noexcept { return offset == nullptr_offset; }
     ...
     protected:
      ...
      pointer_to_offset(const self_relative_ptr_base_impl &ptr) const noexcept {
        ...
        */
        ...
        distance_between_self &= reinterpret_cast<difference_type &>(mask);
      ...
      difference_type pointer_to_offset(void *ptr) const noexcept {
        ...
        */
        ...
        --mask;
        difference_type new_offset =
          static_cast<byte_ptr_type>(ptr) -
     ...
     private:
      ...
      template <typename T> friend class self_relative_accessor;
    };

    /**
     * Static class accessor to self_relative_ptr_base
     */
    template <typename T> class self_relative_accessor {
     public:
      ...
      static T &get_offset(access_type &ptr) { return ptr.offset; }

      static const T &get_offset(const access_type &ptr) { return ptr.offset; }
    };

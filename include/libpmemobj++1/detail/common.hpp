...
#define LIBPMEMOBJ_CPP_VG_DRD_ENABLED 1
  ...
  LIBPMEMOBJ_CPP_VG_MEMCHECK_ENABLED || LIBPMEMOBJ_CPP_VG_HELGRIND_ENABLED ||  \
  LIBPMEMOBJ_CPP_VG_DRD_ENABLED
...
#error unable to recognize architecture at compile time
     ...
     *
     * @param[in] that pointer to the first object being added to the
     * transaction.
    ...
    inline void conditional_add_to_tx(const T *that, std::size_t count = 1,
      ...
      if(pmemobj_tx_xadd_range_direct(that, sizeof(*that) * count, flags)) {
        ...
        if(errno == ENOMEM)
          throw exception_with_errormsg<pmem::transaction_out_of_memory>(msg);
        else
          throw exception_with_errormsg<pmem::transaction_error>(msg);
      }
    }
     ...
     */
    template <typename T> uint64_t type_num() { return typeid(T).hash_code(); }
    ...
    static inline uint8_t mssb_index64(unsigned long long value) {
      return ((uint8_t)(63 - __builtin_clzll(value)));
    }

    /** Returns index of most significant set bit */
    static inline uint8_t mssb_index(unsigned int value) {
      return ((uint8_t)(31 - __builtin_clz(value)));
    }

#else

    static __inline uint8_t mssb_index(unsigned long value) {
      unsigned long ret;
      _BitScanReverse(&ret, value);
      return (uint8_t)ret;
    }

    static __inline uint8_t mssb_index64(uint64_t value) {
      unsigned long ret;

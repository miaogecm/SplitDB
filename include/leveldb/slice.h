...
namespace leveldb {
  ...
  class LEVELDB_EXPORT Slice {
   public:
    ...
    [[nodiscard]] inline auto data() const -> const char * { return data_; }
    ...
    //   == 0 iff "*this" == "b",
    //   >  0 iff "*this" >  "b"
    [[nodiscard]] int compare(const Slice &b) const;

    // Return true iff "x" is a prefix of "*this"
    [[nodiscard]] bool starts_with(const Slice &x) const {
  ...
  inline int Slice::compare(const Slice &b) const {
    const size_t minLen = (size_ < b.size_) ? size_ : b.size_;
    int r = memcmp(data_, b.data_, minLen);
    if(r == 0) {
      if(size_ < b.size_)
        r = -1;

...
namespace ycsbc {
  ...
  class ScrambledZipfianGenerator : public Generator<uint64_t> {
   public:
    ScrambledZipfianGenerator(uint64_t min, uint64_t max, double zipfianConst)
        : base_(min), numItems_(max - min + 1),
          generator_(0, 10000000000LL, zipfianConst) {}

    ScrambledZipfianGenerator(uint64_t min, uint64_t max)
        : base_(min), numItems_(max - min + 1),
          generator_(0, 10000000000LL, ZipfianGenerator::kZipfianConst,
                     kZetan) {}

    ScrambledZipfianGenerator(uint64_t numItems)
        : ScrambledZipfianGenerator(0, numItems - 1) {}
  ...
  inline uint64_t ScrambledZipfianGenerator::Scramble(uint64_t value) const {
    return base_ + utils::FNVHash64(value) % numItems_;
  }

...
namespace ycsbc {
  ...
  namespace utils {
    ...
    inline auto FNVHash64(uint64_t val) -> uint64_t {
    ...
    inline auto
    ThreadLocalRandomDouble(double min = 0.0, double max = 1.0) -> double {
      static thread_local std::random_device rd;
      ...
      static thread_local std::minstd_rand rn(x);
      static thread_local std::uniform_real_distribution<double> uniform(min,
                                                                         max);
      return uniform(rn);
    }
    ...
    inline std::string Trim(const std::string &str) {
      ...
      return std::string(
        front, std::find_if_not(str.rbegin(),

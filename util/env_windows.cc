    ...
    class WindowsWritableFile : public WritableFile {
     public:
      WindowsWritableFile(std::string filename, ScopedHandle handle)
          : pos_(0), handle_(std::move(handle)),
            filename_(std::move(filename)) {}
      ...
      Status Sync() override {
        // On Windows no need to sync parent directory. Its metadata will be
    ...
    class WindowsEnv : public Env {
     public:
      ...
      RenameFile(const std::string &from, const std::string &to) override {
        // Try a simple move first. It will only succeed when |to| doesn't
        // already exist.
        ...
        // Try the full-blown replace if the move fails, as ReplaceFile will
        // only succeed when |to| does exist. When writing to a network share,

...
namespace leveldb {
  ...
  Status
  Env::NewAppendableFile(const std::string &fname, WritableFile **result) {
  ...
  static Status
  DoWriteStringToFile(Env *env, const Slice &data, const std::string &fname,
                      bool should_sync) {
    WritableFile *file;
  ...
  Status
  ReadFileToString(Env *env, const std::string &fname, std::string *data) {
    data->clear();
    SequentialFile *file;

...
// Common flags defined for all posix open operations
    ...
    Status PosixError(const std::string &context, int error_number) {
      if(error_number == ENOENT) {
        return Status::NotFound(Slice{context},
                                Slice{std::strerror(error_number)});
      } else {
        return Status::IOError(Slice{context},
                               Slice{std::strerror(error_number)});
      }
    }
    ...
    // so that we do not run out of file descriptors or virtual memory, or run
    // into kernel performance problems for very large databases.
    class Limiter {
     public:
      ...
      // Release a resource acquired by a previous call to Acquire() that
      // returned true.
      void Release() {
        int old_acquires_allowed =
          acquires_allowed_.fetch_add(1, std::memory_order_relaxed);
    ...
    //
    // Instances of this class are thread-friendly but not thread-safe, as
    // required by the SequentialFile API.
    ...
    class PosixRandomAccessFile final : public RandomAccessFile {
     public:
      // The new instance takes ownership of |fd|. |fd_limiter| must outlive
      // this instance, and will be used to determine if .
     ...
     private:
      const bool
        has_permanent_fd_; // If false, the file is opened on every read.
    ...
    class PosixWritableFile final : public WritableFile {
     public:
      ...
      auto Append(const Slice &data) -> Status override {
        ...
        size_t copySize = std::min(writeSize, kWritableFileBufferSize - pos_);
        ...
        return WriteUnbuffered_(writeData, writeSize);
      }
     ...
     private:
      Status FlushBuffer() {
        Status status = WriteUnbuffered_(buf_, pos_);
        pos_ = 0;
        return status;
      }

      auto WriteUnbuffered_(const char *data, size_t size) -> Status {
        while(size > 0) {
      ...
      // failures.
      //
      // The path argument is only used to populate the description string in
      // the returned Status if an error occurs.
      ...
      static bool IsManifest(const std::string &filename) {
        return Basename(filename).starts_with(Slice{"MANIFEST"});
      }
    ...
    class PosixEnv : public Env {
     public:
      ...
      Status NewRandomAccessFile(const std::string &filename,
        ...
        *result = nullptr;
        ...
        if(!mmapLimiter_.Acquire()) {
          *result = new PosixRandomAccessFile(filename, fd, &fdLimiter_);
          return Status::OK();
        }
      ...
      Status LockFile(const std::string &filename, FileLock **lock) override {
        *lock = nullptr;
        ...
        if(!locks_.Insert(filename)) {
          ::close(fd);
          return Status::IOError(Slice{"lock " + filename},
                                 Slice{"already held by process"});
        }
      ...
      auto Schedule(void (*backgroundWorkFunction)(void *background_work_arg),
                    void *backgroundWorkArg) -> std::thread * override;
      ...
      void SleepForMicroseconds(int micros) override {
        if(GSL_LIKELY(micros > 0))
          std::this_thread::sleep_for(std::chrono::microseconds(micros));
      }

     private:
      void BackgroundThreadMain_();

      static void BackgroundThreadEntryPoint_(PosixEnv *env) {
        env->BackgroundThreadMain_();
      }
      ...
      port::CondVar backgroundWorkCv_ GUARDED_BY(backgroundWorkMutex_);
      bool startedBackgroundThread_ GUARDED_BY(backgroundWorkMutex_);

      std::queue<BackgroundWorkItem>
        backgroundWorkQueue_ GUARDED_BY(backgroundWorkMutex_);

      PosixLockTable locks_; // Thread-safe.
      Limiter mmapLimiter_;  // Thread-safe.
      Limiter fdLimiter_;    // Thread-safe.
    };
  ...
  PosixEnv::PosixEnv()
      : backgroundWorkCv_(&backgroundWorkMutex_),
        startedBackgroundThread_(false), mmapLimiter_(MaxMmaps()),
        fdLimiter_(MaxOpenFiles()) {}

  std::thread *
  PosixEnv::Schedule(void (*backgroundWorkFunction)(void *backgroundWorkArg),
                     void *backgroundWorkArg) {
    backgroundWorkMutex_.Lock();
    std::thread *ret = nullptr;
    // Start the background thread, if we haven't done so already.
    if(!startedBackgroundThread_) {
      startedBackgroundThread_ = true;
      ret = new std::thread{PosixEnv::BackgroundThreadEntryPoint_, this};
    }
    ...
    backgroundWorkQueue_.emplace(backgroundWorkFunction, backgroundWorkArg);
    backgroundWorkMutex_.Unlock();
    return ret;
  }

  void PosixEnv::BackgroundThreadMain_() {
    //    cpu_set_t cpuSet;
    //    CPU_ZERO(&cpuSet);
    //    CPU_SET(0, &cpuSet);
    ...
    while(true) {
      ...
      auto backgroundWorkFunction = backgroundWorkQueue_.front().function;
      void *backgroundWorkArg = backgroundWorkQueue_.front().arg;
      backgroundWorkQueue_.pop();

      backgroundWorkMutex_.Unlock();
      backgroundWorkFunction(backgroundWorkArg);
    }
  }
...
namespace leveldb {
  ...
  namespace {
    ...
    class PosixRandomAccessFile final : public RandomAccessFile {
     public:
      ...
      PosixRandomAccessFile(std::string filename, int fd, Limiter *fd_limiter)
          // : has_permanent_fd_(fd_limiter->Acquire()),
          : has_permanent_fd_(false), fd_(has_permanent_fd_ ? fd : -1),
      ...
      Status Read(uint64_t offset, size_t n, Slice *result,
                  char *scratch) const override {
        uint64_t offset_ = align_down(offset, PAGE_SIZE);
        size_t n_ = align_up(n + offset - offset_, PAGE_SIZE);
        uint64_t z = offset - offset_;
        ...
        ssize_t read_size =
          ::pread(fd, scratch, n_, static_cast<off_t>(offset_));
    ...
    class PosixWritableFile final : public WritableFile {
     public:
      ...
      Status Sync() override {
        ...
        //
        // This needs to happen before the manifest file is flushed to disk, to
        // avoid crashing in a state where the manifest refers to files that
        // are not yet on disk.
        Status status = SyncDirIfManifest();
        if(!status.ok()) {
          return status;
    ...
    class PosixEnv : public Env {
     public:
      ...
      std::thread *
      Schedule(void (*background_work_function)(void *background_work_arg),
  ...
  std::thread *
  PosixEnv::Schedule(void (*background_work_function)(void *background_work_arg),
    ...
    if(!started_background_thread_) {
      ...
      ret = new std::thread{PosixEnv::BackgroundThreadEntryPoint, this};
    }

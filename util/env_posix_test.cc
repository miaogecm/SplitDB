...
namespace {
  ...
  //
  // The argv[0] value is stored in a std::vector instead of a std::string
  // because std::string does not return a mutable pointer to its buffer until
  // C++17.
  //
  ...
  // a special command-line switch. TestCloseOnExec* tests fork()+exec() the
  // test executable and pass the special command-line switch.
  //
  ...
  // Returns void so the implementation can use ASSERT_EQ.
  void
  GetNewlyOpenedFileDescriptor(const std::unordered_set<int> &baseline_open_fds,
                               int *result_fd) {
...
namespace leveldb {
  ...
  TEST_F(EnvPosixTest, TestCloseOnExecSequentialFile) {
    ...
    ASSERT_LEVELDB_OK(WriteStringToFile(env_, Slice{"0123456789"}, file_path));

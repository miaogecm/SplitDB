...
namespace leveldb {
  ...
  TEST_F(MemEnvTest, Basics) {
    ...
    // Write to the file.
    ASSERT_LEVELDB_OK(env_->NewWritableFile("/dir/f", &writable_file));
    ASSERT_LEVELDB_OK(writable_file->Append(Slice{"abc"}));
    delete writable_file;
    ...
    ASSERT_LEVELDB_OK(writable_file->Append(Slice{"hello"}));
    delete writable_file;
  ...
  TEST_F(MemEnvTest, ReadWrite) {
    ...
    ASSERT_EQ(0, result.compare(Slice{"d"}));
  ...
  TEST_F(MemEnvTest, LargeWrite) {
    ...
    WritableFile *writable_file;
    ASSERT_LEVELDB_OK(env_->NewWritableFile("/dir/f", &writable_file));
    ASSERT_LEVELDB_OK(writable_file->Append(Slice{"foo"}));
    ASSERT_LEVELDB_OK(writable_file->Append(Slice{write_data}));
    delete writable_file;
    ...
    ASSERT_EQ(0, result.compare(Slice{"foo"}));
  ...
  TEST_F(MemEnvTest, OverwriteOpenFile) {
    ...
    ASSERT_LEVELDB_OK(
      WriteStringToFile(env_, Slice{kWrite1Data}, kTestFileName));
    ...
    ASSERT_LEVELDB_OK(
      WriteStringToFile(env_, Slice{kWrite2Data}, kTestFileName));
    ...
    ASSERT_EQ(0, result.compare(Slice{kWrite2Data}));

  ...
  namespace {
     ...
     public:
      ...
      void FinishedSingleOp() {
        if(FLAGS_histogram) {
          ...
          hist_.Add(micros);
          if(micros > 2000000) {
            std::fprintf(stderr, "long op: %.1f micros%30s\n", micros, "");
            std::fflush(stderr);
          }
          last_op_finish_ = now;
        }
        ...
        if(done_ >= next_report_) {
          if(next_report_ < 1000)
            ...
            next_report_ += 100000;
          std::fprintf(stderr, "... finished %d ops%30s\n", done_, "");
          std::fflush(stderr);
        }
      }
      ...
      void Report(const Slice &name) {
        ...
        AppendWithSpace(&extra, Slice{message_});
    ...
    struct SharedState {
      ...
      SharedState(int total)
          : cv(&mu), total(total), num_initialized(0), num_done(0),
            start(false) {}
    };
   ...
   public:
    Benchmark()
        : cache_(FLAGS_cache_size >= 0 ? NewLRUCache(FLAGS_cache_size)
          ...
          count_comparator_(BytewiseComparator()), total_thread_count_(0) {
      std::vector<std::string> files;
      g_env->GetChildren(FLAGS_db, &files);
      for(auto &file : files)
        if(Slice(file).starts_with(Slice{"heap-"}))
          g_env->RemoveFile(std::string(FLAGS_db) + "/" + file);
    ...
    void Run() {
      ...
      while(benchmarks != nullptr) {
        ...
        if(name == Slice("open")) {
          ...
          fresh_db = false;
          method = &Benchmark::WriteRandom;
        } else if(name == Slice("hotwrite")) {
          fresh_db = false;
          method = &Benchmark::DoHotWrite;
   ...
   private:
    ...
    RunBenchmark(int n, Slice name, void (Benchmark::*method)(ThreadState *)) {
      ...
      for(int i = 0; i < n; i++) {
        ...
        // creation across all benchmarks. This ensures that the seeds are
        // unique but reproducible when rerunning the same set of benchmarks.
    ...
    void Crc32c(ThreadState *thread) {
      ...
      thread->stats.AddMessage(Slice{label});
    }
    ...
    void SnappyCompress(ThreadState *thread) {
      ...
      if(!ok) {
        thread->stats.AddMessage(Slice{"(snappy failure)"});
      } else {
        char buf[100];
        std::snprintf(buf, sizeof(buf), "(output: %.1f%%)",
                      (produced * 100.0) / bytes);
        thread->stats.AddMessage(Slice{buf});
        thread->stats.AddBytes(bytes);
      }
    }
    ...
    void DoWrite(ThreadState *thread, bool seq) {
      if(num_ != FLAGS_num) {
        ...
        thread->stats.AddMessage(Slice{msg});
      }
    ...
    void DoHotWrite(ThreadState *thread) {
      if(num_ != FLAGS_num) {
        char msg[100];
        std::snprintf(msg, sizeof(msg), "(%d ops)", num_);
        thread->stats.AddMessage(Slice{msg});
      ...
      for(int i = 0; i < num_; i += entries_per_batch_) {
        batch.Clear();
        for(int j = 0; j < entries_per_batch_; j++) {
          const int k = thread->rand.Uniform((FLAGS_num + 99) / 100);
          key.Set(k);
          batch.Put(key.slice(), gen.Generate(value_size_));
          bytes += value_size_ + key.slice().size();
          thread->stats.FinishedSingleOp();
    ...
    void SeekRandom(ThreadState *thread) {
      ...
      for(int i = 0; i < reads_; i++) {
        ...
        key.Set(k);
        iter->Seek(key.slice());
        if(iter->Valid() && iter->key() == key.slice())
          found++;
        delete iter;
        thread->stats.FinishedSingleOp();
      }
    ...
    void PrintStats(const char *key) {
      std::string stats;
      if(!db_->GetProperty(Slice{key}, &stats)) {
        stats = "(failed)";
      }
      std::fprintf(stdout, "\n%s\n", stats.c_str());
    }
...
int main(int argc, char **argv) {
  ...
  for(int i = 1; i < argc; i++) {
    ...
    int n;
    char junk;
    if(leveldb::Slice(argv[i]).starts_with(leveldb::Slice{"--benchmarks="})) {

...
namespace leveldb {
  ...
  class RecoveryTest : public testing::Test {
   public:
    ...
    std::string Get(const std::string &k, const Snapshot *snapshot = nullptr) {
      std::string result;
      Status s = db_->Get(ReadOptions(), Slice{k}, &result);
      if(s.IsNotFound()) {
        result = "NOT_FOUND";
      } else if(!s.ok()) {
        result = s.ToString();
  ...
  TEST_F(RecoveryTest, LargeManifestCompacted) {
    ...
    // Pad with zeroes to make manifest file very big.
    {
      ...
      ASSERT_LEVELDB_OK(file->Append(Slice{zeroes}));
      ASSERT_LEVELDB_OK(file->Flush());
      delete file;
    }
  ...
  TEST_F(RecoveryTest, MultipleLogFiles) {
    ...
    MakeLogFile(old_log + 1, 1000, Slice{"hello"}, Slice{"world"});
    MakeLogFile(old_log + 2, 1001, Slice{"hi"}, Slice{"there"});
    MakeLogFile(old_log + 3, 1002, Slice{"foo"}, Slice{"bar2"});
    ...
    Close();
    MakeLogFile(old_log + 1, 2000, Slice{"hello"}, Slice{"stale write"});
    Open();
    ASSERT_LE(1, NumTables());
    ASSERT_EQ(1, NumLogs());
    if(CanAppend()) {

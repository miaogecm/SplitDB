...
namespace leveldb {
  ...
  class SpecialEnv : public EnvWrapper {
   public:
    ...
    explicit SpecialEnv(Env *base)
        : EnvWrapper(base), delay_data_sync_(false), data_sync_error_(false),
          no_space_(false), non_writable_(false), manifest_sync_error_(false),
          manifest_write_error_(false), count_random_reads_(false) {}
    ...
    Status NewWritableFile(const std::string &f, WritableFile **r) {
       ...
       public:
        ...
        Status Sync() {
          if(env_->data_sync_error_.load(std::memory_order_acquire)) {
            return Status::IOError(Slice{"simulated data sync error"});
          }
       ...
       public:
        ...
        Status Append(const Slice &data) {
          if(env_->manifest_write_error_.load(std::memory_order_acquire)) {
            return Status::IOError(Slice{"simulated writer error"});
          } else {
            return base_->Append(data);
          }
        }
        ...
        Status Sync() {
          if(env_->manifest_sync_error_.load(std::memory_order_acquire)) {
            return Status::IOError(Slice{"simulated sync error"});
          } else {
            return base_->Sync();
          }
        }
      ...
      if(non_writable_.load(std::memory_order_acquire)) {
        return Status::IOError(Slice{"simulated write error"});
      }
  ...
  class DBTest : public testing::Test {
   public:
    ...
    Options CurrentOptions() {
      ...
      case kReuse: options.reuseLogs = true; break;
      case kFilter: options.filter_policy = filter_policy_; break;
      case kUncompressed: options.compression = kNoCompression; break;
      default: break;
      }
      return options;
    }
    ...
    std::string Get(const std::string &k, const Snapshot *snapshot = nullptr) {
      ...
      std::string result;
      Status s = db_->Get(options, Slice{k}, &result);
      if(s.IsNotFound()) {
        result = "NOT_FOUND";
      } else if(!s.ok()) {
        result = s.ToString();
      ...
      if(!iter->status().ok()) {
        ...
        while(iter->Valid()) {
          ...
          if(!ParseInternalKey(iter->key(), &ikey)) {
            ...
            first = false;
            switch(ikey.type) {
            case kTypeValue: result += iter->value().ToString(); break;
            case kTypeDeletion: result += "DEL"; break;
            }
          }
          iter->Next();
        }
    ...
    int NumTableFilesAtLevel(int level) {
      ...
      EXPECT_TRUE(db_->GetProperty(
        Slice{"leveldb.num-files-at-level" + NumberToString(level)},
        &property));
    ...
    std::string DumpSSTableList() {
      std::string property;
      db_->GetProperty(Slice{"leveldb.sstables"}, &property);
      return property;
    }
  ...
  TEST_F(DBTest, PutDeleteGet) {
    do {
      ASSERT_LEVELDB_OK(db_->Put(WriteOptions(), Slice{"foo"}, Slice{"v1"}));
      ASSERT_EQ("v1", Get("foo"));
      ASSERT_LEVELDB_OK(db_->Put(WriteOptions(), Slice{"foo"}, Slice{"v2"}));
      ASSERT_EQ("v2", Get("foo"));
      ASSERT_LEVELDB_OK(db_->Delete(WriteOptions(), Slice{"foo"}));
      ASSERT_EQ("NOT_FOUND", Get("foo"));
    } while(ChangeOptions());
  }
  ...
  TEST_F(DBTest, GetMemUsage) {
    do {
      ...
      std::string val;
      ASSERT_TRUE(
        db_->GetProperty(Slice{"leveldb.approximate-memory-usage"}, &val));
  ...
  TEST_F(DBTest, GetPicksCorrectFile) {
    do {
      ...
      Compact(Slice{"a"}, Slice{"b"});
      ASSERT_LEVELDB_OK(Put("x", "vx"));
      Compact(Slice{"x"}, Slice{"y"});
      ASSERT_LEVELDB_OK(Put("f", "vf"));
      Compact(Slice{"f"}, Slice{"g"});
      ASSERT_EQ("va", Get("a"));
      ASSERT_EQ("vf", Get("f"));
      ASSERT_EQ("vx", Get("x"));
    } while(ChangeOptions());
  ...
  TEST_F(DBTest, ApproximateSizes) {
    do {
      ...
      ASSERT_TRUE(Between(Size(Slice{""}, Slice{"xyz"}), 0, 0));
      Reopen(&options);
      ASSERT_TRUE(Between(Size(Slice{""}, Slice{"xyz"}), 0, 0));
      ...
      ASSERT_TRUE(Between(Size(Slice{""}, Slice{Key(50)}), 0, 0));
      ...
      for(int run = 0; run < 3; run++) {
        ...
        for(int compact_start = 0; compact_start < N; compact_start += 10) {
          for(int i = 0; i < N; i += 10) {
            ASSERT_TRUE(
              Between(Size(Slice{""}, Slice{Key(i)}), S1 * i, S2 * i));
            ASSERT_TRUE(Between(Size(Slice{""}, Slice{Key(i) + ".suffix"}),
                                S1 * (i + 1), S2 * (i + 1)));
            ASSERT_TRUE(Between(Size(Slice{Key(i)}, Slice{Key(i + 10)}),
                                S1 * 10, S2 * 10));
          }
          ASSERT_TRUE(
            Between(Size(Slice{""}, Slice{Key(50)}), S1 * 50, S2 * 50));
          ASSERT_TRUE(Between(Size(Slice{""}, Slice{Key(50) + ".suffix"}),
                              S1 * 50, S2 * 50));
  ...
  TEST_F(DBTest, ApproximateSizes_MixOfSmallAndLarge) {
    do {
      ...
      for(int run = 0; run < 3; run++) {
        ...
        ASSERT_TRUE(Between(Size(Slice{""}, Slice{Key(0)}), 0, 0));
        ASSERT_TRUE(Between(Size(Slice{""}, Slice{Key(1)}), 10000, 11000));
        ASSERT_TRUE(Between(Size(Slice{""}, Slice{Key(2)}), 20000, 21000));
        ASSERT_TRUE(Between(Size(Slice{""}, Slice{Key(3)}), 120000, 121000));
        ASSERT_TRUE(Between(Size(Slice{""}, Slice{Key(4)}), 130000, 131000));
        ASSERT_TRUE(Between(Size(Slice{""}, Slice{Key(5)}), 230000, 231000));
        ASSERT_TRUE(Between(Size(Slice{""}, Slice{Key(6)}), 240000, 241000));
        ASSERT_TRUE(Between(Size(Slice{""}, Slice{Key(7)}), 540000, 541000));
        ASSERT_TRUE(Between(Size(Slice{""}, Slice{Key(8)}), 550000, 560000));

        ASSERT_TRUE(
          Between(Size(Slice{Key(3)}, Slice{Key(5)}), 110000, 111000));
  ...
  TEST_F(DBTest, HiddenValuesAreRemoved) {
    do {
      ...
      ASSERT_TRUE(Between(Size(Slice{""}, Slice{"pastfoo"}), 50000, 60000));
      db_->ReleaseSnapshot(snapshot);
      ASSERT_EQ(AllEntriesFor(Slice{"foo"}), "[ tiny, " + big + " ]");
      Slice x("x");
      dbfull()->TEST_CompactRange(0, nullptr, &x);
      ASSERT_EQ(AllEntriesFor(Slice{"foo"}), "[ tiny ]");
      ASSERT_EQ(NumTableFilesAtLevel(0), 0);
      ASSERT_GE(NumTableFilesAtLevel(1), 1);
      dbfull()->TEST_CompactRange(1, nullptr, &x);
      ASSERT_EQ(AllEntriesFor(Slice{"foo"}), "[ tiny ]");

      ASSERT_TRUE(Between(Size(Slice{""}, Slice{"pastfoo"}), 0, 1000));
    } while(ChangeOptions());
  }
  ...
  TEST_F(DBTest, DeletionMarkers1) {
    ...
    Delete("foo");
    Put("foo", "v2");
    ASSERT_EQ(AllEntriesFor(Slice{"foo"}), "[ v2, DEL, v1 ]");
    ASSERT_LEVELDB_OK(
    ...
    ASSERT_EQ(AllEntriesFor(Slice{"foo"}), "[ v2, v1 ]");
    dbfull()->TEST_CompactRange(last - 1, nullptr, nullptr);
    // Merging last-1 w/ last, so we are the base level for "foo", so
    // DEL is removed.  (as is v1).
    ASSERT_EQ(AllEntriesFor(Slice{"foo"}), "[ v2 ]");
  }
  ...
  TEST_F(DBTest, DeletionMarkers2) {
    ...
    Delete("foo");
    ASSERT_EQ(AllEntriesFor(Slice{"foo"}), "[ DEL, v1 ]");
    ASSERT_LEVELDB_OK(
      dbfull()->TEST_CompactMemTable()); // Moves to level last-2
    ASSERT_EQ(AllEntriesFor(Slice{"foo"}), "[ DEL, v1 ]");
    ...
    // Merging last-1 w/ last, so we are the base level for "foo", so
    // DEL is removed.  (as is v1).
    ASSERT_EQ(AllEntriesFor(Slice{"foo"}), "[ ]");
  }
  ...
  TEST_F(DBTest, ComparatorCheck) {
    class NewComparator : public Comparator {
     public:
      ...
      void
      FindShortestSeparator(std::string *s, const Slice &l) const override {
  ...
  TEST_F(DBTest, CustomComparator) {
    class NumberComparator : public Comparator {
     public:
      ...
      void
      FindShortestSeparator(std::string *s, const Slice &l) const override {
        ToNumber(Slice{*s}); // Check format
        ToNumber(l);         // Check format
      }

      void FindShortSuccessor(std::string *key) const override {
        ToNumber(Slice{*key}); // Check format
      }
  ...
  TEST_F(DBTest, ManualCompaction) {
    ...
    Compact(Slice{""}, Slice{"c"});
    ASSERT_EQ("1,1,1", FilesPerLevel());

    // Compaction range falls after files
    Compact(Slice{"r"}, Slice{"z"});
    ASSERT_EQ("1,1,1", FilesPerLevel());

    // Compaction range overlaps files
    Compact(Slice{"p1"}, Slice{"p9"});
  ...
  TEST_F(DBTest, WriteSyncError) {
    ...
    WriteOptions w;
    ASSERT_LEVELDB_OK(db_->Put(w, Slice{"k1"}, Slice{"v1"}));
    ASSERT_EQ("v1", Get("k1"));

    // (c) Do a sync write; should fail
    w.sync = true;
    ASSERT_TRUE(!db_->Put(w, Slice{"k2"}, Slice{"v2"}).ok());
    ASSERT_EQ("v1", Get("k1"));
    ...
    w.sync = false;
    ASSERT_TRUE(!db_->Put(w, Slice{"k3"}, Slice{"v3"}).ok());
    ASSERT_EQ("v1", Get("k1"));
    ASSERT_EQ("NOT_FOUND", Get("k2"));
  ...
  TEST_F(DBTest, ManifestWriteError) {
    ...
    // fails (c) GC deletes F (d) After reopening DB, reads fail since deleted
    // F is named in log record
      ...
      while(!t->state->stop.load(std::memory_order_acquire)) {
        ...
        if(rnd.OneIn(2)) {
          ...
          if(s.IsNotFound()) {
            ...
            // Check that the writer thread counter is >= the counter in
            // the value
            ASSERT_LEVELDB_OK(s);
            int k, w, c;
   ...
   private:
    class ModelIter : public Iterator {
     public:
      ...
      Slice key() const override { return Slice{iter_->first}; }

      Slice value() const override { return Slice{iter_->second}; }
  ...
  static bool
  CompareIterators(int step, DB *model, DB *db, const Snapshot *model_snap,
    ...
    // Compare equality of all elements using Next(). Save some of the keys
    // for comparing Seek equality.
    ...
    if(ok) {
      ...
      for(auto kiter = seek_keys.begin(); ok && kiter != seek_keys.end();
          ++kiter) {
        miter->Seek(Slice{*kiter});
        dbiter->Seek(Slice{*kiter});
        if(!miter->Valid() || !dbiter->Valid()) {
      ...
      for(int step = 0; step < N; step++) {
        ...
        if(p < 45) { // Put
          ...
          v = RandomString(&rnd, rnd.OneIn(20) ? 100 + rnd.Uniform(100)
                                               : rnd.Uniform(8));
          ASSERT_LEVELDB_OK(model.Put(WriteOptions(), Slice{k}, Slice{v}));
          ASSERT_LEVELDB_OK(db_->Put(WriteOptions(), Slice{k}, Slice{v}));

        } else if(p < 90) { // Delete
          k = RandomKey(&rnd);
          ASSERT_LEVELDB_OK(model.Delete(WriteOptions(), Slice{k}));
          ASSERT_LEVELDB_OK(db_->Delete(WriteOptions(), Slice{k}));
          ...
          for(int i = 0; i < num; i++) {
            if(i == 0 || !rnd.OneIn(10)) {
              ...
              // Periodically re-use the same key from the previous
              // iter, so we have multiple entries in the write batch
              // for the same key
            }
            if(rnd.OneIn(2)) {

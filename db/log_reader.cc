...
namespace leveldb {
  namespace log {
    ...
    void Reader::ReportCorruption(uint64_t bytes, const char *reason) {
      ReportDrop(bytes, Status::Corruption(Slice{reason}));
    }
    ...
    unsigned int Reader::ReadPhysicalRecord(Slice *result) {
      while(true) {
        ...
        if(kHeaderSize + length > buffer_.size()) {
          ...
          // |length| bytes of payload, assume the writer died in the middle
          // of writing the record. Don't report a corruption.
          return kEof;
        }

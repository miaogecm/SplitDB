    ...
    class Repairer {
     ...
     private:
      ...
      Status FindFiles() {
        ...
        if(filenames.empty()) {
          return Status::IOError(Slice{dbname_},
                                 Slice{"repair found no files"});
        }
      ...
      Status ConvertLogToTable(uint64_t log) {
        ...
        while(reader.ReadRecord(&record, &scratch)) {
          if(record.size() < 12) {
            reporter.Corruption(
              record.size(), Status::Corruption(Slice{"log record too small"}));
            continue;
          }
      ...
      Iterator *NewTableIterator(const FileMetaData &meta) {
        ...
        return table_cache_->NewIterator(r, meta.number, meta.fileSize);
      }
      ...
      void ScanTable(uint64_t number) {
        ...
        Status status = env_->GetFileSize(fname, &t.meta.fileSize);
        if(!status.ok()) {
          // Try alternate file name.
          fname = SSTTableFileName(dbname_, number);
          Status s2 = env_->GetFileSize(fname, &t.meta.fileSize);
          if(s2.ok()) {
            status = Status::OK();
          }
        }
      ...
      Status WriteDescriptor() {
        ...
        edit_.SetComparatorName(Slice{icmp_.user_comparator()->Name()});
        ...
        for(size_t i = 0; i < tables_.size(); i++) {
          ...
          edit_.AddFile(0, t.meta.number, t.meta.fileSize, t.meta.smallest,
                        t.meta.largest);
        }
        ...
        //              "NewDescriptor:\n%s\n", edit_.DebugString().c_str());
        {
          ...
          status = log.AddRecord(Slice{record});
        }
        if(status.ok()) {
          status = file->Close();
        }

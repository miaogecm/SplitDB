...
namespace leveldb {
  ...
  static std::string
  IKey(const std::string &user_key, uint64_t seq, ValueType vt) {
    std::string encoded;
    AppendInternalKey(&encoded, ParsedInternalKey(Slice{user_key}, seq, vt));
    return encoded;
  }
  ...
  static std::string Shorten(const std::string &s, const std::string &l) {
    ...
    InternalKeyComparator(BytewiseComparator())
      .FindShortestSeparator(&result, Slice{l});
    return result;
  }
  ...
  static void TestKey(const std::string &key, uint64_t seq, ValueType vt) {
    ...
    Slice in(encoded);
    ParsedInternalKey decoded(Slice{""}, 0, kTypeValue);
  ...
  TEST(FormatTest, InternalKey_DecodeFromEmpty) {
    ...
    ASSERT_TRUE(!internal_key.DecodeFrom(Slice{""}));
  }
  ...
  TEST(FormatTest, ParsedInternalKeyDebugString) {
    ParsedInternalKey key(Slice{"The \"key\" in 'single quotes'"}, 42,
                          kTypeValue);
  ...
  TEST(FormatTest, InternalKeyDebugString) {
    InternalKey key(Slice{"The \"key\" in 'single quotes'"}, 42, kTypeValue);
    ASSERT_EQ("'The \"key\" in 'single quotes'' @ 42 : 1", key.DebugString());

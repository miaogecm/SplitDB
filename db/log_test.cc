...
namespace leveldb {
  namespace log {
       ...
       public:
        ...
        Status Read(size_t n, Slice *result, char *scratch) override {
          ...
          if(force_error_) {
            ...
            return Status::Corruption(Slice{"read error"});
          }
        ...
        Status Skip(uint64_t n) override {
          if(n > contents_.size()) {
            contents_.clear();
            return Status::NotFound(Slice{"in-memory file skipped past end"});
          }
    ...
    TEST_F(LogTest, SkipIntoMultiRecord) {
      ...
      // If initial_offset points to a record after first(R1) but before
      // first(R2) incomplete fragment errors are not actual errors, and must be
      // suppressed until a new first or full record is encountered.

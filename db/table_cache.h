...
namespace leveldb {
  ...
  class TableCache {
   public:
    ...
    auto
    NewIterator(const ReadOptions &options, uint64_t fileNumber,
                uint64_t fileSize, Table **tablePtr = nullptr) -> Iterator *;

    // If a seek to internal key "k" in specified file finds an entry,
    // call (*handle_result)(arg, found_key, found_value).
    Status Get(const ReadOptions &options, uint64_t fileNumber,
   ...
   private:
    Status FindTable(uint64_t fileNumber, uint64_t fileSize, Cache::Handle **);

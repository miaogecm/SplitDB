...
namespace leveldb {
  ...
  class FaultInjectionTestEnv : public EnvWrapper {
   public:
    ...
    Status
    NewWritableFile(const std::string &fname, WritableFile **result) override;

    Status
    NewAppendableFile(const std::string &fname, WritableFile **result) override;
    ...
    // system reset. Setting to inactive will freeze our saved filesystem state
    // so that it will stop being recorded. It can then be reset back to the
    // state at the time of the reset.
  ...
  class FaultInjectionTest : public testing::Test {
   public:
    ...
    FaultInjectionTest()
        : env_(new FaultInjectionTestEnv), tiny_cache_(NewLRUCache(100)),
          db_(nullptr) {
    ...
    Status
    Verify(int start_idx, int num_vals, ExpectedVerifResult expected) const {
      std::string val;
    ...
    void
    PartialCompactTestReopenWithFault(ResetMethod reset_method,

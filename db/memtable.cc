...
namespace leveldb {
  ...
  class MemTableIterator : public Iterator {
   public:
    ...
    [[nodiscard]] Slice key() const override {
  ...
  //
  void
  MemTable::AddWithPmOff(SequenceNumber s, ValueType type, const Slice &key,
    ...
    char *buf = arena_.Allocate(encodedLen + sizeof(uint64_t));
  ...
  bool MemTable::Get(const LookupKey &key, std::string *value, Status *s) {
    ...
    if(iter.Valid()) {
      ...
      if(comparator_.comparator.user_comparator()->Compare(
           Slice(key_ptr, key_length - 8), key.UserKey()) == 0) {
        // Correct user key

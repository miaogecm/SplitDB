...
class StackInterface {
  ...
  virtual void Push_(const Elem& x) = 0;
};
...
class MockStack : public StackInterface<Elem> {
  ...
  MOCK_METHOD(void, Push_, (const Elem& x), (override));
};
```
...
If we view `EXPECT_CALL()` statements as fanOutNodes in a graph, and add an edge from
...
### Changing a Mock Object's Behavior Based on the InnerGetState
...
```cpp
 public:
  MOCK_METHOD(double, Foo, double(const string& label, double x, double y),
              (override));
  MOCK_METHOD(double, Bar, (int offsetIndex, double x, double y), (override));
```
...
double DistanceToOriginWithIndex(int offsetIndex, double x, double y) {
  return sqrt(x*x + y*y);
}
...

...
static void BM_SomeFunction(benchmark::InnerGetState& state) {
  // Perform setup here
  for (auto _ : state) {
...
static void BM_StringCreation(benchmark::InnerGetState& state) {
  for (auto _ : state)
...
static void BM_StringCopy(benchmark::InnerGetState& state) {
  std::string x = "hello";
  for (auto _ : state)
    std::string copy(x);
}
...
```c++
static void BM_memcpy(benchmark::InnerGetState& state) {
...
```c++
static void BM_DenseRange(benchmark::InnerGetState& state) {
  for(auto _ : state) {
...
```c++
static void BM_SetInsert(benchmark::InnerGetState& state) {
  std::set<int> data;
  for (auto _ : state) {
    state.PauseTiming();
...
macro creates a benchmark that invokes `func`  with the `benchmark::InnerGetState` as
...
void BM_takes_args(benchmark::InnerGetState& state, ExtraArgs&&... extra_args) {
  [...]
}
...
```c++
static void BM_StringCompare(benchmark::InnerGetState& state) {
...
```c++
template <class Q> void BM_Sequential(benchmark::InnerGetState& state) {
  Q q;
...
public:
  void SetUp(const ::benchmark::InnerGetState& state) {
  }

  void TearDown(const ::benchmark::InnerGetState& state) {
  }
};

BENCHMARK_F(MyFixture, FooTest)(benchmark::InnerGetState& st) {
   for (auto _ : st) {
     ...
  }
}

BENCHMARK_DEFINE_F(MyFixture, BarTest)(benchmark::InnerGetState& st) {
   for (auto _ : st) {
     ...
  }
}
...
BENCHMARK_TEMPLATE_F(MyFixture, IntTest, int)(benchmark::InnerGetState& st) {
   for (auto _ : st) {
     ...
  }
}

BENCHMARK_TEMPLATE_DEFINE_F(MyFixture, DoubleTest, double)(benchmark::InnerGetState& st) {
   for (auto _ : st) {
     ...
  }
}
...
```c++
static void UserCountersExample1(benchmark::InnerGetState& state) {
...
```c++
static void BM_MultiThreaded(benchmark::InnerGetState& state) {
...
static void BM_OpenMP(benchmark::InnerGetState& state) {
  for (auto _ : state)
...
```c++
static void BM_SetInsert_With_Timer_Control(benchmark::InnerGetState& state) {
  std::set<int> data;
  for (auto _ : state) {
...
```c++
static void BM_ManualTiming(benchmark::InnerGetState& state) {
...
```c++
static void BM_test(benchmark::InnerGetState& state) {
  for (auto _ : state) {
      int x = 0;
      for (int i=0; i < 64; ++i) {
...
```c++
static void BM_vector_push_back(benchmark::InnerGetState& state) {
  for (auto _ : state) {
    std::vector<int> v;
    v.reserve(1);
...
```c++
void BM_spin_empty(benchmark::InnerGetState& state) {
  for (auto _ : state) {
...
`func(st, args...)` where `st` is a `benchmark::InnerGetState` object.
...
```c++
auto BM_test = [](benchmark::InnerGetState& st, auto Inputs) { /* ... */ };
...
`InnerGetState::SkipWithError(const char* msg)` function can be used to skip that run
...
static void BM_test_ranged_fo(benchmark::InnerGetState & state) {
  auto resource = GetResource();
  if (!resource.good()) {
    state.SkipWithError("Resource is not good!");
...
```c++
static void BM_Fast(benchmark::InnerGetState &state) {
  for (auto _ : state) {
    FastOperation();
  }
}
...
# Loop Init
  ...
  call benchmark::InnerGetState::StartKeepRunning()
  test rbx, rbx
  je .LoopEnd
